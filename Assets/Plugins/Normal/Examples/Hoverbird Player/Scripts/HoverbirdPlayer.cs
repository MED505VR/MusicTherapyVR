...
namespace Normal.Realtime.Examples
{
    public class HoverbirdPlayer : MonoBehaviour
    {
        ...
        private void CalculateTargetMovement()
        {
            ...
            var cameraLookForwardVector = ProjectVectorOntoGroundPlane(cameraTarget.forward);
            var cameraLookForward = Quaternion.LookRotation(cameraLookForwardVector);
        ...
        private void AnimateCharacter()
        {
            ...
            var characterLocalForwardVector = _character.localRotation * Vector3.forward;
            var characterLookForwardVector = ProjectVectorOntoGroundPlane(characterLocalForwardVector);
            var characterLookForward = Quaternion.LookRotation(characterLookForwardVector);

            // Calculate the angle between the current movement direction and the target movement direction
            var targetMovementNormalized = _targetMovement.normalized;
            var movementNormalized = _movement.normalized;
            var angle = targetMovementNormalized.sqrMagnitude > 0.0f
                ? SignedAngle2D(targetMovementNormalized, movementNormalized)
                : 0.0f;
            ...
            var leanRotation = characterLookForward * Quaternion.Euler(0.0f, 0.0f, angle);

            // Rotate to face the direction of travel if we're moving forward
            var targetCharacterLookForwardVector = characterLookForwardVector;
            ...
            var targetRotation = Quaternion.LookRotation(targetCharacterLookForwardVector, leanRotation * Vector3.up);

            // Animate the character towards the target rotation
            _character.localRotation =
                Quaternion.Slerp(_character.localRotation, targetRotation, 5.0f * Time.fixedDeltaTime);
        }
        ...
        {
            var forwardVelocity = rigidbody.velocity;
        ...
        {
            var angle = Mathf.Atan2(a.z, a.x) - Mathf.Atan2(b.z, b.x);
