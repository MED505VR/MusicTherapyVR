...
public class OVRProjectConfigEditor : Editor
{
    ...
    {
        var projectConfig = (OVRProjectConfig)target;
    ...
    public static void DrawTargetDeviceInspector(OVRProjectConfig projectConfig)
    {
        ...
        {
            var oldSupportsDevice = projectConfig.targetDeviceTypes.Contains(deviceType);
            var newSupportsDevice = oldSupportsDevice;
            ...
            else if (oldSupportsDevice && !newSupportsDevice) projectConfig.targetDeviceTypes.Remove(deviceType);
        }

        if (hasModified) OVRProjectConfig.CommitProjectConfig(projectConfig);
    }
    ...
    public static void DrawProjectConfigInspector(OVRProjectConfig projectConfig)
    {
        ...
        OVREditorUtil.SetupEnumField(projectConfig, "Hand Tracking Support", ref projectConfig.handTrackingSupport,
            ref hasModified);
        ...
        OVREditorUtil.SetupInputField(projectConfig, "Custom Security XML Path", ref projectConfig.securityXmlPath,
            ref hasModified);
        OVREditorUtil.SetupBoolField(projectConfig, "Disable Backups", ref projectConfig.disableBackups,
            ref hasModified);
        OVREditorUtil.SetupBoolField(projectConfig, "Enable NSC Configuration", ref projectConfig.enableNSCConfig,
            ref hasModified);
