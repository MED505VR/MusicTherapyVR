...
public class OVRRaycaster : GraphicRaycaster, IPointerEnterHandler
{
    ...
    /// world objects
    /// </summary>
    [NonSerialized] private List<RaycastHit> m_RaycastResults = new List<RaycastHit>();
        ...
        bool checkForBlocking)
    {
        ...
        if (checkForBlocking && blockingObjects != BlockingObjects.None)
        {
            ...
            if (blockingObjects == BlockingObjects.ThreeD || blockingObjects == BlockingObjects.All)
            {
                ...
                if (hits.Length > 0 && hits[0].distance < hitDistance) hitDistance = hits[0].distance;
            }
            ...
            if (blockingObjects == BlockingObjects.TwoD || blockingObjects == BlockingObjects.All)
            {
                ...
                if (hits.Length > 0 && hits[0].fraction * dist < hitDistance) hitDistance = hits[0].fraction * dist;
            }
        }
    ...
    {
        if (eventData.IsVRPointer()) Raycast(eventData, resultAppendList, eventData.GetRay(), true);
    }
    ...
    private void GraphicRaycast(Canvas canvas, Ray ray, List<RaycastHit> results)
    {
        ...
        for (var i = 0; i < s_SortedGraphics.Count; ++i) results.Add(s_SortedGraphics[i]);
    }
