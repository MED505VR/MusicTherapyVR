...
public class OVRMixedRealityCaptureTest : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (!defaultExternalCamera) Debug.LogWarning("defaultExternalCamera undefined");

#if !OVR_ANDROID_MRC
        // On Quest, we enable MRC automatically through the configuration
        if (!OVRManager.instance.enableMixedReality) OVRManager.instance.enableMixedReality = true;
#endif
#endif
    }
    ...
    private void UpdateDefaultExternalCamera()
    {
        ...
        var cameraName = "UnityExternalCamera";
        var cameraIntrinsics = new OVRPlugin.CameraIntrinsics();
        var cameraExtrinsics = new OVRPlugin.CameraExtrinsics();
        ...
        var vFov = defaultExternalCamera.fieldOfView * Mathf.Deg2Rad;
        var hFov = Mathf.Atan(Mathf.Tan(vFov * 0.5f) * cameraAspect) * 2.0f;
        var fov = new OVRPlugin.Fovf();
        ...
        if (cameraRig)
        {
            ...
            var trackingSpacePose = trackingSpace.ToOVRPose(false);
            var cameraPose = defaultExternalCamera.transform.ToOVRPose(false);
            var relativePose = trackingSpacePose.Inverse() * cameraPose;
#if OVR_ANDROID_MRC
 ...
 OVRPlugin.GetTrackingTransformRelativePose(OVRPlugin.TrackingOrigin.Stage).ToOVRPose();
    ...
    private void Update()
    {
        ...
        if (!defaultExternalCamera) return;
...
#else
        if (!OVRPlugin.IsMixedRealityInitialized()) return;
#endif
        ...
        else if (currentMode == CameraMode.OverrideFov)
        {
            ...
            if (!OVRPlugin.GetUseOverriddenExternalCameraFov(0)) Debug.LogWarning("FOV not overridden");
        }
        else if (currentMode == CameraMode.ThirdPerson)
        {
            ...
            var hFov = Mathf.Atan(Mathf.Tan(vFov * 0.5f) * camera.aspect) * 2.0f;
            var fov = new OVRPlugin.Fovf();
            fov.UpTan = fov.DownTan = Mathf.Tan(vFov * 0.5f);
            fov.LeftTan = fov.RightTan = Mathf.Tan(hFov * 0.5f);
            ...
            if (cameraRig)
            {
                var trackingSpace = cameraRig.trackingSpace;
                var trackingSpacePose = trackingSpace.ToOVRPose(false);
                var cameraPose = transform.ToOVRPose(false);
                var relativePose = trackingSpacePose.Inverse() * cameraPose;
                var stageToLocalPose = OVRPlugin.GetTrackingTransformRelativePose(OVRPlugin.TrackingOrigin.Stage)
                    .ToOVRPose();
                var localToStagePose = stageToLocalPose.Inverse();
                var relativePoseInStage = localToStagePose * relativePose;
                var relativePosef = relativePoseInStage.ToPosef();
            ...
            if (!OVRPlugin.GetUseOverriddenExternalCameraStaticPose(0)) Debug.LogWarning("StaticPose not overridden");
        }
#endif
    }
}