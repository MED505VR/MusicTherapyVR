...
public class OVRSkeleton : MonoBehaviour
{
    ...
    [SerializeField] protected SkeletonType _skeletonType = SkeletonType.None;
    ...
    private void Awake()
    {
        if (_dataProvider == null) _dataProvider = GetComponent<IOVRSkeletonDataProvider>();
    ...
    {
        var flipX = _skeletonType == SkeletonType.HandLeft || _skeletonType == SkeletonType.HandRight;
        ...
        {
            var bone = _bones[i] ?? (_bones[i] = new OVRBone());
            bone.Id = (BoneId)_skeleton.Bones[i].Id;
            bone.ParentBoneIndex = _skeleton.Bones[i].ParentBoneIndex;

            var trans = bone.Transform ?? (bone.Transform = new GameObject(bone.Id.ToString()).transform);
            trans.localPosition =
                flipX
    ...
    private void InitializeBindPose()
    {
        ...
        // pre-populate bones list before attempting to apply bone hierarchy
        for (var i = 0; i < _bindPoses.Count; ++i)
        {
            var bone = _bones[i];
            var bindPoseBone = _bindPoses[i] ?? (_bindPoses[i] = new OVRBone());
            bindPoseBone.Id = bone.Id;
            bindPoseBone.ParentBoneIndex = bone.ParentBoneIndex;

            var trans = bindPoseBone.Transform ??
                        (bindPoseBone.Transform = new GameObject(bindPoseBone.Id.ToString()).transform);
    ...
    private void InitializeCapsules()
    {
        ...
        if (_enablePhysicsCapsules)
        {
            ...
            {
                var bone = _bones[_skeleton.BoneCapsules[i].BoneIndex];
                var capsule = _capsules[i] ?? (_capsules[i] = new OVRBoneCapsule());
                capsule.BoneIndex = _skeleton.BoneCapsules[i].BoneIndex;

                if (capsule.CapsuleRigidbody == null)
                {
                    capsule.CapsuleRigidbody =
                        new GameObject(bone.Id.ToString() + "_CapsuleRigidbody").AddComponent<Rigidbody>();
                ...
                var rbGO = capsule.CapsuleRigidbody.gameObject;
                ...
                var ccGO = capsule.CapsuleCollider.gameObject;
    ...
    private void Update()
    {
        ...
        if (data.IsDataValid)
        {
            ...
            if (_updateRootScale) transform.localScale = new Vector3(data.RootScale, data.RootScale, data.RootScale);
    ...
    private void FixedUpdate()
    {
        ...
        if (_enablePhysicsCapsules)
        {
            ...
            for (var i = 0; i < _capsules.Count; ++i)
            {
                ...
                {
                    if (capsuleGO.activeSelf) capsuleGO.SetActive(false);
                }
            }
        }
    }
