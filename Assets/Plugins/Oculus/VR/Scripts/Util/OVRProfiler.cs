...
public class OVRProfiler : EditorWindow
{
    ...
    private static TargetPlatform mTargetPlatform;
    ...
    private void OnGUI()
    {
        ...
        mTargetPlatform =
            (TargetPlatform)EditorGUILayout.Popup("Target Oculus Platform", (int)mTargetPlatform, options);
    ...
    private void UpdateRecords()
    {
        ...
        else
        {
            ...
            AddRecord("Graphics Memory (MB)", "Please use less than 1024 MB of vertex and texture memory.",
                ConvertBytes(memSizeByte, "MB"), 0, 1024);
        }
    ...
    {
        return string.Format("{0:0.##} {1}", ConvertBytes(bytes, target), target);
    }
    ...
    {
        var record = new RangedRecord(category, message, value, min, max);
        mRecords.Add(record);
    }
}
