...
public class OVRMeshRenderer : MonoBehaviour
{
    ...
    [SerializeField] private IOVRMeshRendererDataProvider _dataProvider;
    ...
    private void Awake()
    {
        if (_dataProvider == null) _dataProvider = GetComponent<IOVRMeshRendererDataProvider>();

        if (_ovrMesh == null) _ovrMesh = GetComponent<OVRMesh>();
    ...
    private void Initialize()
    {
        ...
        if (!_skinnedMeshRenderer) _skinnedMeshRenderer = gameObject.AddComponent<SkinnedMeshRenderer>();
        ...
        {
            var numSkinnableBones = _ovrSkeleton.GetCurrentNumSkinnableBones();
            var bindPoses = new Matrix4x4[numSkinnableBones];
            var bones = new Transform[numSkinnableBones];
            var localToWorldMatrix = transform.localToWorldMatrix;
            for (var i = 0; i < numSkinnableBones && i < _ovrSkeleton.Bones.Count; ++i)
            {
