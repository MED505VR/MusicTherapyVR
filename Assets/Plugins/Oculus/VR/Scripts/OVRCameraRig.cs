...
public class OVRCameraRig : MonoBehaviour
{
    ...
    protected virtual void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
    {
        ...
        var emulatedRotation = Quaternion.Euler(-OVRManager.instance.headPoseRelativeOffsetRotation.x,
        ...
        if (updateEyeAnchors)
        {
            if (hmdPresent)
            {
                var centerEyePosition = Vector3.zero;
                var centerEyeRotation = Quaternion.identity;
            ...
            {
                var leftEyePosition = Vector3.zero;
                var rightEyePosition = Vector3.zero;
                var leftEyeRotation = Quaternion.identity;
                var rightEyeRotation = Quaternion.identity;
        ...
        if (updateHandAnchors)
        {
            ...
            if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
            {
                ...
                var leftQuat = Quaternion.identity;
                var rightQuat = Quaternion.identity;
                ...
                if (OVRNodeStateProperties.GetNodeStatePropertyQuaternion(Node.LeftHand,
                    NodeStatePropertyType.Orientation, OVRPlugin.Node.HandLeft, OVRPlugin.Step.Render, out leftQuat))
                    leftHandAnchor.localRotation = leftQuat;
                if (OVRNodeStateProperties.GetNodeStatePropertyQuaternion(Node.RightHand,
                    NodeStatePropertyType.Orientation, OVRPlugin.Node.HandRight, OVRPlugin.Step.Render, out rightQuat))
    ...
    protected virtual void OnBeforeRenderCallback()
    {
        ...
        {
            var controllersNeedUpdate = OVRManager.instance.LateControllerUpdate;
#if USING_XR_SDK
