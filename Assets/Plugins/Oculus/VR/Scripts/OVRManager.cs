...
#pragma warning restore
    ...
    [SerializeField] [HideInInspector] private ColorSpace _colorGamut = ColorSpace.Rift_CV1;
    ...
    {
        get => OVRPlugin.eyeFovPremultipliedAlphaModeEnabled;
    ...
    {
        get => instantiateMixedRealityCameraGameObject;
    ...
    public static float gpuUtilLevel
    {
        get
        {
            if (!OVRPlugin.gpuUtilSupported) Debug.LogWarning("GPU Util is not supported");
    ...
    public static OVRPose GetOpenVRControllerOffset(Node hand)
    {
        ...
        if ((hand == Node.LeftHand || hand == Node.RightHand) && loadedXRDevice == XRDevice.OpenVR)
        {
            ...
            {
                var offsetOrientation = hand == Node.LeftHand
                ...
                poseOffset.position =
                    hand == Node.LeftHand ? OpenVRTouchPositionOffsetLeft : OpenVRTouchPositionOffsetRight;
            }
        }
    ...
    public static bool IsUnityAlphaOrBetaVersion()
    {
        ...
        while (pos >= 0 && ver[pos] >= '0' && ver[pos] <= '9') --pos;
    ...
    private void InitOVRManager()
    {
        ...
        if (xrApi == XrApi.OpenXR) Debug.LogFormat("OpenXR instance 0x{0:X} session 0x{1:X}", xrInstance, xrSession);
        ...
        var supportedTypes =
            ...
            GraphicsDeviceType.Direct3D12.ToString();
 ...
 false;		// we should never start the standalone game in MxR mode, unless the command-line parameter is provided
        ...
        if (!staticMixedRealityCaptureInitialized)
        {
            ...
            if (loadMrcConfig || createMrcConfig)
            {
                ...
                if (createMrcConfig) mrcSettings.WriteToConfigurationFile();
                Destroy(mrcSettings);
            }

            if (MixedRealityEnabledFromCmd()) enableMixedReality = true;
        ...
        if (enableAdaptiveResolution && !IsAdaptiveResSupportedByEngine())
        {
            ...
            Debug.LogError("Your current Unity Engine " + Application.unityVersion +
                           " might have issues to support adaptive resolution, please disable it under OVRManager");
        }
#endif
        ...
        if (Debug.isDebugBuild)
        {
            ...
            if (!perfTcpServer.enabled) perfTcpServer.enabled = true;
            OVRPlugin.SetDeveloperMode(OVRPlugin.Bool.True);
        }

        // Refresh the client color space
        var clientColorSpace = colorGamut;
    ...
    {
#if USING_XR_SDK
        var currentDisplaySubsystem = GetCurrentDisplaySubsystem();
        var currentDisplaySubsystemDescriptor = GetCurrentDisplaySubsystemDescriptor();
#endif
        if (OVRPlugin.initialized)
        {
        ...
        {
#if USING_XR_SDK
            var loadedXRDeviceName = currentDisplaySubsystemDescriptor.id;
#else
    ...
    private void Update()
    {
        ...
        if (AllowRecenter && OVRPlugin.shouldRecenter) display.RecenterPose();
        ...
        if (monoscopic != _monoscopic) monoscopic = _monoscopic;
        ...
        if (enableAdaptiveResolution)
        {
            ...
            var recommendedViewportScale =
                Mathf.Clamp(
                    Mathf.Sqrt(OVRPlugin.GetAdaptiveGPUPerformanceScale()) * Settings.eyeTextureResolutionScale *
                    Settings.renderViewportScale, 0.5f, 2.0f);
        ...
        while (OVRPlugin.PollEvent(ref eventDataBuffer))
            switch (eventDataBuffer.EventType)
            {
                case OVRPlugin.EventType.DisplayRefreshRateChanged:
                    ...
                    {
                        var FromRefreshRate = BitConverter.ToSingle(eventDataBuffer.EventData, 0);
                        var ToRefreshRate = BitConverter.ToSingle(eventDataBuffer.EventData, 4);
    ...
    private static Camera FindMainCamera()
    {
        ...
        foreach (var obj in objects)
        {
            ...
            if (camera != null && camera.enabled)
            {
                ...
                if (cameraRig != null && cameraRig.trackingSpace != null) cameras.Add(camera);
            }
        }
    ...
    {
        var perfTcpServer = GetComponent<OVRSystemPerfMetrics.OVRSystemPerfMetricsTcpServer>();
        if (perfTcpServer != null) perfTcpServer.enabled = false;
    }
    ...
    {
        if (!staticMixedRealityCaptureInitialized) return;
        ...
        if (configuration.enableMixedReality || staticPrevEnableMixedRealityCapture)
        {
            ...
            if (mainCamera != null)
            {
                ...
                if (staticPrevEnableMixedRealityCapture && !configuration.enableMixedReality) OVRMixedReality.Cleanup();
