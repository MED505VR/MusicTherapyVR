...
public class OVRSceneLoader : MonoBehaviour
{
    ...
    private void Start()
    {
        var applicationPath = Path.Combine(externalStoragePath, Application.identifier);
    ...
    private void LoadScene(SceneInfo sceneInfo)
    {
        ...
        var bundles = Directory.GetFiles(scenePath, "*_*");
        logTextBox.text += "Loading " + bundles.Length + " bundle(s) . . . ";
        var mainSceneBundleFileName = "scene_" + sceneInfo.scenes[0].ToLower();
        try
        {
            foreach (var b in bundles)
            {
                ...
                if (assetBundle.name == mainSceneBundleFileName) mainSceneBundle = assetBundle;

                if (assetBundle.name == resourceBundleName) OVRResources.SetResourceBundle(assetBundle);
            }
        }
        catch (Exception e)
        {
        ...
        if (mainSceneBundle != null)
        {
            ...
            var scenePaths = mainSceneBundle.GetAllScenePaths();
    ...
    private void UpdateCanvasPosition()
    {
        ...
        if (mainCanvas.worldCamera != Camera.main)
        {
            ...
            if (Camera.main != null)
            {
                var newPosition = Camera.main.transform.position + Camera.main.transform.forward * 0.3f;
    ...
    private SceneInfo GetSceneInfo()
    {
        ...
        try
        {
            ...
            while (!reader.EndOfStream) sceneList.Add(reader.ReadLine());
    ...
    private IEnumerator onCheckSceneCoroutine()
    {
        ...
        while (true)
        {
            ...
            if (newSceneInfo.version != currentSceneInfo.version)
            {
                ...
                for (var i = 0; i < activeScenes; i++) SceneManager.UnloadSceneAsync(SceneManager.GetSceneAt(i));
    ...
    {
        foreach (var go in Resources.FindObjectsOfTypeAll(typeof(GameObject)) as GameObject[]) Destroy(go);
    }
}