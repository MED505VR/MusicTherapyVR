...
public abstract class OVRCameraComposition : OVRComposition
{
    ...
    protected OVRCameraComposition(GameObject parentObject, Camera mainCamera,
        ...
        : base(parentObject, mainCamera, configuration)
    {
        ...
        Debug.Assert(!OVRPlugin.IsCameraDeviceAvailable(cameraDevice) ||
        ...
        if (OVRPlugin.IsCameraDeviceAvailable(cameraDevice))
        {
            ...
            if (OVRPlugin.GetExternalCameraCount() > 0 &&
                OVRPlugin.GetMixedRealityCameraInfo(0, out extrinsics, out intrinsics))
                OVRPlugin.SetCameraDevicePreferredColorFrameSize(cameraDevice, intrinsics.ImageSensorPixelResolution.w,
    ...
    protected void RefreshCameraFramePlaneObject(GameObject parentObject, Camera mixedRealityCamera,
        OVRMixedRealityCaptureConfiguration configuration)
    {
        OVRCompositionUtil.SafeDestroy(ref cameraFramePlaneObject);
        ...
        cameraFramePlaneObject.transform.parent =
        ...
        var cameraFrameMaterial = new Material(Shader.Find(configuration.useDynamicLighting
    ...
    protected void UpdateCameraFramePlaneObject(Camera mainCamera, Camera mixedRealityCamera,
        OVRMixedRealityCaptureConfiguration configuration, RenderTexture boundaryMeshMaskTexture)
    {
        cameraFrameCompositionManager.configuration = configuration;
        var hasError = false;
        ...
        if (!hasError)
        {
            ...
            cameraFramePlaneObject.transform.position = mixedRealityCamera.transform.position +
            ...
            cameraFramePlaneObject.transform.localScale =
                new Vector3(distance * mixedRealityCamera.aspect * tanFov * 2.0f, distance * tanFov * 2.0f, 1.0f);
            ...
            cameraFrameMaterial.SetVector("_FlipParams",
                new Vector4(configuration.flipCameraFrameHorizontally ? 1.0f : 0.0f,
                    configuration.flipCameraFrameVertically ? 1.0f : 0.0f, 0.0f, 0.0f));
            ...
            cameraFrameMaterial.SetVector("_TextureDimension",
                new Vector4(colorTexture.width, colorTexture.height, 1.0f / colorTexture.width,
            ...
            cameraFrameMaterial.SetFloat("_DepthVariationClamp",
                configuration.dynamicLightingDepthVariationClampingValue);
            cameraFrameMaterial.SetFloat("_CullingDistance", cullingDistance);
            if (configuration.virtualGreenScreenType == OVRManager.VirtualGreenScreenType.Off || boundaryMesh == null ||
    ...
    protected void RefreshBoundaryMesh(Camera camera, OVRMixedRealityCaptureConfiguration configuration,
        out float cullingDistance)
    {
        var depthTolerance = configuration.virtualGreenScreenApplyDepthCulling
            ? configuration.virtualGreenScreenDepthTolerance
            : float.PositiveInfinity;
        cullingDistance =
            OVRCompositionUtil.GetMaximumBoundaryDistance(camera,
                OVRCompositionUtil.ToBoundaryType(configuration.virtualGreenScreenType)) + depthTolerance;
        if (boundaryMesh == null || boundaryMeshType != configuration.virtualGreenScreenType ||
            ...
            boundaryMeshBottomY != configuration.virtualGreenScreenBottomY)
        {
            ...
            boundaryMesh = OVRCompositionUtil.BuildBoundaryMesh(
                OVRCompositionUtil.ToBoundaryType(configuration.virtualGreenScreenType), boundaryMeshTopY,
                boundaryMeshBottomY);
    ...
    public class OVRCameraFrameCompositionManager : MonoBehaviour
    {
        ...
        private void OnPreRender()
        {
            if (configuration != null &&
                ...
                boundaryMeshMaskTexture != null && composition.boundaryMesh != null)
            {
                ...
                for (var i = 0; i < whiteMaterial.passCount; ++i)
                    if (whiteMaterial.SetPass(i))
                        Graphics.DrawMeshNow(composition.boundaryMesh,
