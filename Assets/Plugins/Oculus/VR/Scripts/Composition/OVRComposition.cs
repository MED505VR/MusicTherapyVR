...
public abstract class OVRComposition
{
    ...
    protected OVRPose
        lastAttachedNodePose =
            new OVRPose(); // Sometimes the attach node pose is not readable (lose tracking, low battery, etc.) Use the last pose instead when it happens
    ...
    public void RefreshCameraRig(GameObject parentObject, Camera mainCamera)
    {
        ...
        if (cameraRig == null) cameraRig = parentObject.GetComponent<OVRCameraRig>();
    ...
    public OVRPose ComputeCameraWorldSpacePose(OVRPlugin.CameraExtrinsics extrinsics)
    {
        ...
        var worldSpacePose = OVRExtensions.ToWorldSpacePose(trackingSpacePose);
