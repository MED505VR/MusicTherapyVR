...
public class OVROverlay : MonoBehaviour
{
    ...
    /// </summary>
    [Tooltip("Specify overlay's type")] public OverlayType currentOverlayType = OverlayType.Overlay;
    ...
    /// </summary>
    [Tooltip("Specify overlay's shape")] public OverlayShape currentOverlayShape = OverlayShape.Quad;
    ...
    private int texturesPerStage => layout == OVRPlugin.LayerLayout.Stereo ? 2 : 1;
        ...
        OVRPlugin.Sizei size, OVRPlugin.OverlayShape shape)
    {
        ...
        var needsSetup = isOverridePending ||
        ...
        if (isExternalSurface)
        {
            if (externalSurfaceObject == IntPtr.Zero)
            {
                ...
                if (externalSurfaceObject != IntPtr.Zero)
                {
                    ...
                    if (externalSurfaceObjectCreated != null) externalSurfaceObjectCreated();
                }
            }
    ...
    public void UpdateTextureRectMatrix()
    {
        ...
        var srcRectLeftConverted = new Rect(srcRectLeft.x,
        ...
        var srcRectRightConverted = new Rect(srcRectRight.x,
        ...
        var destRectLeftConverted = new Rect(destRectLeft.x,
        ...
        var destRectRightConverted = new Rect(destRectRight.x,
    ...
    {
        if (isExternalSurface) return true;
    ...
    private OVRPlugin.LayerDesc GetCurrentLayerDesc()
    {
        ...
        {
            if (textures[0] == null) Debug.LogWarning("textures[0] hasn't been set");
        ...
        if (isProtectedContent) newDesc.LayerFlags |= (int)OVRPlugin.LayerFlags.ProtectedContent;

        if (isExternalSurface) newDesc.LayerFlags |= (int)OVRPlugin.LayerFlags.AndroidSurfaceSwapChain;
    ...
    private Rect GetBlitRect(int eyeId)
    {
        ...
        else
        {
            ...
            var minX = Mathf.Min(srcRectLeft.x, srcRectRight.x);
            var minY = Mathf.Min(srcRectLeft.y, srcRectRight.y);
            var maxX = Mathf.Max(srcRectLeft.x + srcRectLeft.width, srcRectRight.x + srcRectRight.width);
            var maxY = Mathf.Max(srcRectLeft.y + srcRectLeft.height, srcRectRight.y + srcRectRight.height);
...
#if UNITY_ANDROID && !UNITY_EDITOR
				dataIsLinear =
 true; //HACK: Graphics.CopyTexture causes linear->srgb conversion on target write with D3D but not GLES.
#endif
                // PC requries premultiplied Alpha
                var requiresPremultipliedAlpha = !Application.isMobilePlatform;

                var linearToSRGB = !isHdr && dataIsLinear;
                // if the texture needs to be premultiplied, premultiply it unless its already premultiplied
                var premultiplyAlpha = requiresPremultipliedAlpha && !isAlphaPremultiplied;

                var bypassBlit = !linearToSRGB && !premultiplyAlpha && rt != null && rt.format == rtFormat;
                ...
                if (!bypassBlit)
                {
                    ...
                    if (!tempRTDst.IsCreated()) tempRTDst.Create();
                ...
                if (tempRTDst != null) RenderTexture.ReleaseTemporary(tempRTDst);
        ...
        int frameIndex)
    {
        ...
        if (overrideTextureRectMatrix) UpdateTextureRectMatrix();
        var noTextures = isExternalSurface || !NeedsTexturesForShape(currentOverlayShape);
        var isOverlayVisible = OVRPlugin.EnqueueSubmitLayer(overlay, headLocked, noDepthBufferTesting,
            noTextures ? IntPtr.Zero : layerTextures[0].appTexturePtr,
            noTextures ? IntPtr.Zero : layerTextures[rightEyeIndex].appTexturePtr,
            layerId, frameIndex, pose.flipZ().ToPosef_Legacy(), scale.ToVector3f(), layerIndex,
    ...
    private void SetupEditorPreview()
    {
        ...
        if (previewInEditor && previewObject == null)
        {
            ...
            var generator = previewObject.AddComponent<OVROverlayMeshGenerator>();
    ...
    private void InitOVROverlay()
    {
        ...
        if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
        {
            ...
            if (overlay != null)
            {
                var error = overlay.CreateOverlay(OpenVROverlayKey + transform.name, gameObject.name,
    ...
    private void OnDisable()
    {
        ...
        else if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
        {
            ...
            {
                var overlay = OVR.OpenVR.OpenVR.Overlay;
                if (overlay != null) overlay.DestroyOverlay(OpenVROverlayHandle);
    ...
    private bool ComputeSubmit(ref OVRPose pose, ref Vector3 scale, ref bool overlay, ref bool headLocked)
    {
        ...
        {
            var arcAngle = scale.x / scale.z / (float)Math.PI * 180.0f;
            if (arcAngle > 180.0f)
            {
    ...
    {
        var overlayRef = OVR.OpenVR.OpenVR.Overlay;
        if (overlayRef == null)
            return;

        var overlayTex = textures[0];

        if (overlayTex != null)
        {
            var error = overlayRef.ShowOverlay(OpenVROverlayHandle);
            ...
            var tex = new OVR.OpenVR.Texture_t();
            ...
            var textureBounds = new OVR.OpenVR.VRTextureBounds_t();
            ...
            var vecMouseScale = new OVR.OpenVR.HmdVector2_t();
            ...
            var mat44 = Matrix4x4.TRS(pose.position, pose.orientation, Vector3.one);

            var pose34 = mat44.ConvertToHMDMatrix34();
    ...
    private void LateUpdate()
    {
        if (!OVRManager.OVRManagerinitialized)
            return;
        if (!xrDeviceConstructed) InitOVROverlay();
        ...
        var requiresTextures = !isExternalSurface && NeedsTexturesForShape(currentOverlayShape);
        if (currentOverlayType == OverlayType.None ||
            requiresTextures && (textures.Length < texturesPerStage || textures[0] == null))
            return;

        var pose = OVRPose.identity;
        ...
        var newDesc = GetCurrentLayerDesc();
        var isHdr = newDesc.Format == OVRPlugin.EyeTextureFormat.R16G16B16A16_FP;
        ...
        var textureSizesDiffer = !layerDesc.TextureSize.Equals(newDesc.TextureSize) && layerId > 0;
        var needsTextures = NeedsTexturesForShape(currentOverlayShape);
        var needsTextureChanged = NeedsTexturesForShape(prevOverlayShape) != needsTextures;
        ...
        var createdLayer = CreateLayer(newDesc.MipLevels, newDesc.SampleCount, newDesc.Format, newDesc.LayerFlags,
        ...
        if (needsTextures)
        {
            var useMipmaps = newDesc.MipLevels > 1;
            ...
            {
                var stage = frameIndex % stageCount;
        ...
        var isOverlayVisible = SubmitLayer(overlay, headLocked, noDepthBufferTesting, pose, scale, frameIndex);
