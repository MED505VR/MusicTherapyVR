...
public class OVROverlayMeshGenerator : MonoBehaviour
{
    ...
    protected void Awake()
    {
        ...
        if (Camera.main && Camera.main.transform.parent) _CameraRoot = Camera.main.transform.parent;
    ...
    private Rect GetBoundingRect(Rect a, Rect b)
    {
        ...
        var xMax = Mathf.Max(a.x + a.width, b.x + b.width);
        var yMin = Mathf.Min(a.y, b.y);
        var yMax = Mathf.Max(a.y + a.height, b.y + b.height);
    ...
    private void Update()
    {
        ...
        if (_Overlay)
        {
            var shape = _Overlay.currentOverlayShape;
            var position = _CameraRoot ? _Transform.position - _CameraRoot.position : _Transform.position;
            var rotation = _Transform.rotation;
            var scale = _Transform.lossyScale;
            var destRectLeft = _Overlay.overrideTextureRectMatrix ? _Overlay.destRectLeft : new Rect(0, 0, 1, 1);
            var destRectRight = _Overlay.overrideTextureRectMatrix ? _Overlay.destRectRight : new Rect(0, 0, 1, 1);
            var srcRectLeft = _Overlay.overrideTextureRectMatrix ? _Overlay.srcRectLeft : new Rect(0, 0, 1, 1);
            var texture = _Overlay.textures[0];
            ...
            {
                var previewMat = new Material(Shader.Find("Unlit/Transparent"));
    ...
    {
        var thetaU = (theta / (2 * Mathf.PI) - 0.5f) / expand_coef + 0.5f;
        var phiV = phi / Mathf.PI / expand_coef + 0.5f;
        ...
        int longitudes = 128, float expand_coef = 1.0f)
    {
        ...
        var minTheta = Mathf.PI * 2 * rect.x;
        var minPhi = Mathf.PI * (0.5f - rect.y - rect.height);

        var thetaScale = Mathf.PI * 2 * rect.width / longitudes;
        var phiScale = Mathf.PI * rect.height / latitudes;

        for (var j = 0; j < latitudes + 1; j += 1)
        for (var k = 0; k < longitudes + 1; k++)
        {
            var theta = minTheta + k * thetaScale;
            var phi = minPhi + j * phiScale;

            var suv = GetSphereUV(theta, phi, expand_coef);
            uv.Add(new Vector2((suv.x - rect.x) / rect.width, (suv.y - rect.y) / rect.height));
            var vert = GetSphereVert(theta, phi);
        ...
        Quaternion rotation, Vector3 scale, float worldScale = 800, int subQuads = 1, float expand_coef = 1.01f)
    {
        ...
        var vertsPerSide = (subQuads + 1) * (subQuads + 1);

        for (var i = 0; i < (int)CubeFace.COUNT; i++)
        {
            for (var j = 0; j < subQuads + 1; j++)
            for (var k = 0; k < subQuads + 1; k++)
            {
                var u = j / (float)subQuads;
                var v = k / (float)subQuads;

                uv.Add(GetCubeUV((CubeFace)i, new Vector2(u, v), expand_coef));
                var vert = GetCubeVert((CubeFace)i, new Vector2(u, v), expand_coef);
                vert.x = (worldScale * vert.x - position.x) / scale.x;
                vert.y = (worldScale * vert.y - position.y) / scale.y;
                vert.z = (worldScale * vert.z - position.z) / scale.z;
                verts.Add(vert);
            ...
            for (var j = 0; j < subQuads; j++)
            for (var k = 0; k < subQuads; k++)
            {
    ...
    {
        var height = Mathf.Abs(scale.y) * rect.height;
        var radius = scale.z;
        var arcLength = scale.x * rect.width;

        var arcAngle = arcLength / radius;
        var minAngle = scale.x * (-0.5f + rect.x) / radius;

        var columns = Mathf.CeilToInt(longitudes * arcAngle / (2 * Mathf.PI));
        ...
        var triangleWidth = arcLength / columns;
        var ratio = height / triangleWidth;

        var rows = Mathf.CeilToInt(ratio / 2);
