...
public static class OVRExtensions
{
    ...
    public static OVRPose ToWorldSpacePose(OVRPose trackingSpacePose)
    {
        ...
        var ret = Camera.main.transform.ToOVRPose() * poseInHeadSpace;
    ...
    {
        var pose = new OVR.OpenVR.HmdMatrix34_t();
...
public static class OVRNodeStateProperties
{
    ...
    public static bool IsHmdPresent()
    {
        ...
        if (currentDisplaySubsystem != null)
            return
                currentDisplaySubsystem
                    .running; //In 2019.3, this should be changed to currentDisplaySubsystem.isConnected, but this is a fine placeholder for now.
        return false;
        ...
        out Vector3 retVec)
    {
        ...
        {
            if (requestedNodeState.TryGetAcceleration(out retVec)) return true;
        }
        else if (propertyType == NodeStatePropertyType.AngularAcceleration)
        {
            if (requestedNodeState.TryGetAngularAcceleration(out retVec)) return true;
        }
        else if (propertyType == NodeStatePropertyType.Velocity)
        {
            if (requestedNodeState.TryGetVelocity(out retVec)) return true;
        }
        else if (propertyType == NodeStatePropertyType.AngularVelocity)
        {
            if (requestedNodeState.TryGetAngularVelocity(out retVec)) return true;
        }
        else if (propertyType == NodeStatePropertyType.Position)
        {
            if (requestedNodeState.TryGetPosition(out retVec)) return true;
        }
...
public struct OVRPose
{
    ...
    {
        var result = new OVRPlugin.Posef();
