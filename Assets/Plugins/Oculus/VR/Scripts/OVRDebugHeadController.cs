...
{
    [SerializeField] public bool AllowPitchLook = false;
    [SerializeField] public bool AllowYawLook = true;
    [SerializeField] public bool InvertPitch = false;
    [SerializeField] public float GamePad_PitchDegreesPerSec = 90.0f;
    [SerializeField] public float GamePad_YawDegreesPerSec = 90.0f;
    [SerializeField] public bool AllowMovement = false;
    [SerializeField] public float ForwardSpeed = 2.0f;
    [SerializeField] public float StrafeSpeed = 2.0f;
    ...
    private void Update()
    {
        if (AllowMovement)
        {
            ...
            var fwdMove = CameraRig.centerEyeAnchor.rotation * Vector3.forward * gamePad_FwdAxis * Time.deltaTime *
                          ForwardSpeed;
            var strafeMove = CameraRig.centerEyeAnchor.rotation * Vector3.right * gamePad_StrafeAxis * Time.deltaTime *
                             StrafeSpeed;
        ...
        var hasDevice = false;
#if USING_XR_SDK
        var currentDisplaySubsystem = OVRManager.GetCurrentDisplaySubsystem();
        ...
        if (!hasDevice && (AllowYawLook || AllowPitchLook))
        {
            ...
            if (AllowPitchLook)
            {
                ...
                {
                    if (InvertPitch) gamePadPitch *= -1.0f;
