...
namespace Assets.Oculus.VR.Editor
{
    public class OVRPlatformTool : EditorWindow
    {
        ...
        private const string urlPlatformUtil =
            "https://www.oculus.com/download_app/?id=1076686279105243";

        private static Process ovrPlatUtilProcess;
        private Vector2 commandMenuScroll;
        ...
        private static void Init()
        {
            ...
            GetWindow(typeof(OVRPlatformTool));
            ...
            if (string.IsNullOrEmpty(OVRPlatformToolSettings.DebugSymbolsDirectory))
                OVRPlatformToolSettings.DebugSymbolsDirectory =
                    Path.Combine(Application.dataPath, "../Temp/StagingArea/symbols");
        ...
        private void OnGUI()
        {
            ...
            titleContent.text = "OVR Platform Tool";

            var TargetPlatformLabel = new GUIContent("Target Oculus Platform");
            ...
            commandMenuScroll = EditorGUILayout.BeginScrollView(commandMenuScroll, GUILayout.Height(Screen.height / 2));
            {
                ...
                // App ID
                var AppIDLabel = new GUIContent("Oculus Application ID [?]: ",
                ...
                // Releaes Note
                var ReleaseNoteLabel = new GUIContent("Release Note: ");
                ...
                {
                    var BuildDirLabel = new GUIContent("Rift Build Directory [?]: ",
                ...
                {
                    var ApkPathLabel = new GUIContent("Build APK File Path [?]: ",
                    ...
                    var DebugSymbolLabel = new GUIContent("Debug Symbols Directory [?]: ",
                ...
                if (showOptionalCommands)
                {
                    ...
                    {
                        // Launch Parameters
                        var LaunchParamLabel = new GUIContent("Launch Parameters [?]: ",
                        ...
                        var FirewallExceptionLabel = new GUIContent("Firewall Exception [?]: ",
                        ...
                        var GamepadEmulationLabel = new GUIContent("Gamepad Emulation [?]: ",
                        ...
                        if (show2DCommands)
                        {
                            ...
                            // 2D Launch File
                            var LaunchFile2DLabel = new GUIContent("2D Launch File [?]: ",
                            ...
                            var LaunchParam2DLabel = new GUIContent("2D Launch Parameters [?]: ",
                        ...
                        if (showRedistCommands)
                        {
                            ...
                            for (var i = 0; i < OVRPlatformToolSettings.RiftRedistPackages.Count; i++)
                            {
                                var RedistPackageLabel =
                        ...
                        if (showExpansionFileCommands)
                        {
                            ...
                            var LanguagePackLabel = new GUIContent("Language Pack Directory [?]: ",
                    ...
                    else
                    {
                        ...
                        if (showExpansionFileCommands)
                        {
                            ...
                            // OBB File Path
                            var ObbPathLabel = new GUIContent("OBB File Path [?]: ",
                    ...
                    {
                        // Assets Directory
                        var AssetsDirLabel = new GUIContent("Assets Directory [?]: ",
                            "The full path to the directory with DLCs for this build.");
                        var assetsDirectory = MakeFileDirectoryField(AssetsDirLabel,
                        ...
                        if (assetsDirectory != OVRPlatformToolSettings.AssetsDirectory)
                        {
                            ...
                            {
                                var dirInfo = new DirectoryInfo(OVRPlatformToolSettings.AssetsDirectory);
                                var assetFiles = dirInfo.GetFiles();
                ...
                if (EditorGUI.EndChangeCheck()) EditorUtility.SetDirty(OVRPlatformToolSettings.Instance);
            }
            ...
            // Run OVR Lint Option
            EditorGUIUtility.labelWidth = DEFAULT_LABEL_WIDTH;
            var RunOvrLintLabel = new GUIContent("Run OVR Lint (Recommended) [?]: ",
            ...
            var btnTxt = new GUIContent("Upload");
            ...
            if (GUI.Button(rt, btnTxt, GUI.skin.button))
                if (EditorUtility.DisplayDialog("Cancel Upload Process",
                    ...
                    if (ovrPlatUtilProcess != null)
                    {
                        ...
                        log += "Upload process was canceled\n";
                    }
            ...
            EditorGUILayout.SelectableLabel(log, logBoxStyle, GUILayout.Height(position.height - 30));
            EditorGUILayout.EndScrollView();
        }
        ...
        private void OnUpload(TargetPlatform targetPlatform)
        {
            ...
            SetDirtyOnGUIChange();
            var lintCount = 0;
            if (OVRPlatformToolSettings.RunOvrLint) lintCount = OVRLint.RunCheck();
            if (lintCount != 0)
            {
                log += lintCount.ToString() + " lint suggestions are found. \n" +
        ...
        private static void ExecuteCommand(TargetPlatform targetPlatform)
        {
            var dataPath = Application.dataPath;
            ...
            if (genUploadCommand(targetPlatform, out uploadCommand))
            {
                var thread = new Thread(delegate()
                {
                    ...
                    while (activeProcess) Thread.Sleep(100);
                    retryCount = 0;
        ...
        {
            var toolDataPath = dataPath + "/Oculus/VR/Editor/Tools";
            if (!Directory.Exists(toolDataPath)) Directory.CreateDirectory(toolDataPath);

            var platformUtil = toolDataPath + "/ovr-platform-util.exe";
            if (!File.Exists(platformUtil))
            {
                OVRPlugin.SendEvent("oculus_platform_tool", "provision_util");
                var downloadCoroutine = EditorCoroutine.Start(ProvisionPlatformUtil(platformUtil));
        ...
        private static void CheckForUpdate(string dataPath)
        {
            var platformUtilPath = CheckForPlatformUtil(dataPath);
            ...
            ovrPlatUtilProcess.Exited += new EventHandler(
                (s, e) =>
                {
                    if (File.Exists(dataPath + ".ovr-platform-util.exe"))
                    {
                        ...
                        log += "Finished updating platform utility.\n";
                    }
            ...
            ovrPlatUtilProcess.OutputDataReceived += new DataReceivedEventHandler(
                (s, e) =>
                {
                    if (e.Data != null && e.Data.Length != 0 && !e.Data.Contains("\u001b")) log += e.Data + "\n";
                }
            );
            ...
            {
                if (ThrowPlatformUtilStartupError(platformUtilPath)) CheckForUpdate(dataPath);
            }
        }

        private static void LoadRedistPackages(string dataPath)
        {
            ...
            log += "Loading redistributable packages...\n";

            var redistPacks = new List<RedistPackage>();
            ...
            ovrPlatUtilProcess.OutputDataReceived += new DataReceivedEventHandler(
                (s, e) =>
                {
                    if (e.Data != null && e.Data.Length != 0 && !e.Data.Contains("\u001b") && !e.Data.Contains("ID"))
                    {
                        ...
                        var terms = e.Data.Split('|');
                        if (terms.Length == 2)
                        {
                            var redistPack = new RedistPackage(terms[1], terms[0]);
            ...
            try
            {
                ...
                {
                    log += "Successfully updated redistributable packages.\n";
                    OVRPlatformToolSettings.RiftRedistPackages = redistPacks;
                }
                else
                {
                    log += "Redistributable packages up to date.\n";
                }
            }
            catch
            {
                if (ThrowPlatformUtilStartupError(platformUtilPath)) LoadRedistPackages(dataPath);
            }
        }

        private static void Command(TargetPlatform targetPlatform, string dataPath, string uploadCommand)
        {
            var platformUtilPath = CheckForPlatformUtil(dataPath);
            ...
            {
                if (ThrowPlatformUtilStartupError(platformUtilPath)) Command(targetPlatform, dataPath, uploadCommand);
            }
        }
        ...
        private static bool genUploadCommand(TargetPlatform targetPlatform, out string command)
        {
            ...
            switch (targetPlatform)
            {
                ...
                case TargetPlatform.Quest:
                    ...
                    break;
                default:
                    log += "ERROR: Invalid target platform selected";
                    success = false;
                    break;
            }
            ...
            if (targetPlatform == TargetPlatform.Rift)
            {
                ...
                if (OVRPlatformToolSettings.RiftFirewallException) command += " --firewall_exceptions true";

                // Add Redistributable Packages
                var redistCommandIds = new List<string>();
                for (var i = 0; i < OVRPlatformToolSettings.RiftRedistPackages.Count; i++)
            ...
            if (!string.IsNullOrEmpty(OVRPlatformToolSettings.AssetsDirectory))
            {
                ...
                {
                    var assetConfigs = new List<string>();
                    for (var i = 0; i < OVRPlatformToolSettings.AssetConfigs.Count; i++)
                    {
                        var configParameters = new List<string>();
                        var config = OVRPlatformToolSettings.AssetConfigs[i];

                        if (config.required) configParameters.Add("\\\"required\\\":true");
            ...
            {
                var sanatizedReleaseNote = OVRPlatformToolSettings.ReleaseNote;
        ...
        private static void ValidateTextField(FieldValidatorDelegate<string, string, bool> fieldValidator,
            string fieldText, string fieldName, ref bool success)
        {
            var error = "";
            if (!fieldValidator(fieldText, ref error))
            {
                log += "ERROR: Please verify that the " + fieldName + " is correct. ";
                log += string.IsNullOrEmpty(error) ? "\n" : error + "\n";
                success = false;
            }
        }
        ...
        private static bool AppIDFieldValidator(string fieldText, ref string error)
        {
            if (string.IsNullOrEmpty(fieldText))
            {
                error = "The field is empty.";
        ...
        private static bool ThrowPlatformUtilStartupError(string utilPath)
        {
            ...
            {
                retryCount++;
                log += "There was a problem starting Oculus Platform Util. Restarting provision process...\n";
            ...
            string extension = "")
        {
            ...
            var rect = GUILayoutUtility.GetRect(0, SINGLE_LINE_SPACING, GUILayout.ExpandWidth(true));
            ...
            if (!string.IsNullOrEmpty(variable))
            {
                ...
                if (GUI.Button(rect, "X")) result = string.Empty;
        ...
        private static IEnumerator ProvisionPlatformUtil(string dataPath)
        {
            ...
            {
                log += "Completed Provisioning Oculus Platform Util\n";
            }
        ...
        {
            var elementRect = new Rect(rect.x, rect.y + SINGLE_LINE_SPACING + ASSET_CONFIG_BACKGROUND_PADDING / 2,
            ...
            if (OVRPlatformToolSettings.AssetConfigs.Count > 0)
                for (var i = 0; i < OVRPlatformToolSettings.AssetConfigs.Count; i++)
                {
                    ...
                    {
                        var attributeRect = new Rect(elementRect.x + INDENT_SPACING,
        ...
        {
            var headerRect = new Rect(rect.x, rect.y, rect.width, SINGLE_LINE_SPACING);
        ...
        {
            var backgroundRect = new Rect(rect.x, rect.y + SINGLE_LINE_SPACING, rect.width,
        ...
        private class GUIHelper
        {
            ...
            private static void InOut(Worker begin, Worker body, Worker end)
            {
                try
                {
                    begin();
