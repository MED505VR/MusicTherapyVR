...
/// </summary>
internal class OVRScreenshotWizard : ScriptableWizard
{
    ...
    /// </summary>
    private static bool CreateAssetPath(string newFolderPath)
    {
        ...
        for (var i = 1; i < pathFolders.Length; i++)
            ...
            {
                var newPath = currentPath + "/" + pathFolders[i];
    ...
    private void OnWizardCreate()
    {
        ...
        var existingCamera = true;
        var existingCameraStateSave = true;
        var camera = renderFrom.GetComponent<Camera>();
        if (camera == null)
        {
        ...
        if (cubeMapFolder[cubeMapFolder.Length - 1] != '/') cubeMapFolder += "/";
        var idx = 0;
        var fileNames = Directory.GetFiles(cubeMapFolder);
        foreach (var fileName in fileNames)
        {
            if (!fileName.ToLower().EndsWith(".cubemap")) continue;
            var temp = fileName.Replace(cubeMapFolder + "vr_screenshot_", string.Empty);
        ...
        var pathName = string.Format("{0}vr_screenshot_{1}.cubemap", cubeMapFolder, (++idx).ToString("d2"));
    ...
    /// </summary>
    private void GenerateTexture(Cubemap cubemap, string pathName)
    {
        ...
        var format = textureFormat.ToString();
        var fullPath = EditorUtility.SaveFilePanel(string.Format("Save Cubemap Screenshot as {0}", format), "",
    ...
    private static void TakeOVRScreenshot()
    {
        var wizard = DisplayWizard<OVRScreenshotWizard>("OVR Screenshot Wizard", "Render Cubemap");
        if (wizard != null)
        {
