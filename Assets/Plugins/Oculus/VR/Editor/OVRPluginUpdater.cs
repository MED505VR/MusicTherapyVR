...
internal class OVRPluginUpdater
{
    ...
    private class PluginPackage
    {
        ...
        public bool IsEnabled()
        {
            ...
            var path = "";
            if (Plugins.TryGetValue(PluginPlatform.Win64, out path)) return File.Exists(path);
        ...
        public bool IsAndroidUniversalEnabled()
        {
            ...
            if (Plugins.TryGetValue(PluginPlatform.AndroidUniversal, out path))
                if (File.Exists(path))
                {
                    var basePath = GetCurrentProjectPath();
                    var relPath = path.Substring(basePath.Length + 1);

                    var pi = AssetImporter.GetAtPath(relPath) as PluginImporter;
                    if (pi != null) return pi.GetCompatibleWithPlatform(BuildTarget.Android);
                }
        ...
        public bool IsAndroidUniversalPresent()
        {
            ...
            if (Plugins.TryGetValue(PluginPlatform.AndroidUniversal, out path))
            {
                var disabledPath = path + GetDisabledPluginSuffix();

                if (File.Exists(path) || File.Exists(disabledPath)) return true;
            }
    ...
    private static void OnDelayCall()
    {
        ...
        if (enableAndroidUniversalSupport) unityVersionSupportsAndroidUniversal = true;

        if (ShouldAttemptPluginUpdate()) AttemptPluginUpdate(true);
    }
    ...
    private static List<PluginPackage> GetAllUtilitiesPluginPackages()
    {
        ...
        if (Directory.Exists(pluginRootPath))
        {
            ...
            foreach (var dir in dirs) packages.Add(GetPluginPackage(dir));
        }
    ...
    {
        var basePath = EditorApplication.applicationContentsPath;
        var pluginPath = @"/UnityExtensions/Unity/VR";
    ...
    {
        var pluginVersion = invalidVersion;
        ...
        if (pluginVersion == invalidVersion)
        {
            ...
            if (!File.Exists(path))
            {
                ...
                if (!File.Exists(path)) return invalidVersion;
            }

            var pluginVersionInfo = FileVersionInfo.GetVersionInfo(path);
            if (pluginVersionInfo == null || pluginVersionInfo.ProductVersion == null ||
                pluginVersionInfo.ProductVersion == "") return invalidVersion;
    ...
    {
        var allUtilsPluginPkgs = GetAllUtilitiesPluginPackages();

        foreach (var pluginPkg in allUtilsPluginPkgs)
        foreach (var path in pluginPkg.Plugins.Values)
            if (Directory.Exists(path) || File.Exists(path))
            {
                var basePath = GetCurrentProjectPath();
                var relPath = path.Substring(basePath.Length + 1);
                var relDisabledPath = relPath + GetDisabledPluginSuffix();
    ...
    private static void ActivateOVRPluginOpenXR()
    {
        ...
        if (!unityRunningInBatchmode)
        {
            var accepted = EditorUtility.DisplayDialog("Warning",
        ...
        {
            UnityEngine.Debug.LogError(
                "Unable to Activate OVRPlugin with OpenXR: AndroidOpenXR/OVRPlugin.aar does not exist");
            return;
        }
        ...
        if (enabledUtilsPluginPkg.IsAndroidOpenXREnabled())
        {
            if (!unityRunningInBatchmode)
                EditorUtility.DisplayDialog("Unable to Activate OVRPlugin with OpenXR",
                    "AndroidOpenXR/OVRPlugin.aar already enabled", "Ok");
            return;
        }

        if (enabledUtilsPluginPkg.IsAndroidUniversalEnabled())
        {
            var androidUniveralPluginPath = enabledUtilsPluginPkg.Plugins[PluginPlatform.AndroidUniversal];
            var androidUniveralPluginBasePath = GetCurrentProjectPath();
            var androidUniveralPluginRelPath =
                androidUniveralPluginPath.Substring(androidUniveralPluginBasePath.Length + 1);
            var pi = AssetImporter.GetAtPath(androidUniveralPluginRelPath) as PluginImporter;
            if (pi != null)
            {
                pi.SetCompatibleWithPlatform(BuildTarget.Android, false);
        ...
        {
            var androidOpenXRPluginPath = enabledUtilsPluginPkg.Plugins[PluginPlatform.AndroidOpenXR];
            var androidOpenXRPluginBasePath = GetCurrentProjectPath();
            var androidOpenXRPluginRelPath = androidOpenXRPluginPath.Substring(androidOpenXRPluginBasePath.Length + 1);
            var pi = AssetImporter.GetAtPath(androidOpenXRPluginRelPath) as PluginImporter;
            if (pi != null)
            {
    ...
    private static void RestoreStandardOVRPlugin()
    {
        ...
        {
            UnityEngine.Debug.LogError(
                "Unable to Restore Standard Oculus Utilities Plugin: AndroidOpenXR/OVRPlugin.aar does not exist");
            return;
        }
        ...
        {
            if (!unityRunningInBatchmode)
                EditorUtility.DisplayDialog("Unable to Restore Standard Oculus Utilities Plugin",
                    "AndroidUniversal/OVRPlugin.aar already enabled", "Ok");
            return;
        }
        ...
        if (!unityRunningInBatchmode)
            EditorUtility.DisplayDialog("Restore Standard OVRPlugin",
                "Standard version of Oculus Utilities Plugin has been enabled on Android", "Ok");
    }
    ...
    public static bool IsOVRPluginOpenXRActivated()
    {
        ...
        if (enabledUtilsPluginPkg == null) return false;
    ...
    private static void AttemptPluginUpdate(bool triggeredByAutoUpdate)
    {
        ...
        var bundledPluginPkg = GetBundledPluginPackage();
        var allUtilsPluginPkgs = GetAllUtilitiesPluginPackages();
        ...
        var reenableCurrentPluginPkg = false;
        ...
        var currentPluginPkg = enabledUtilsPluginPkg != null ? enabledUtilsPluginPkg : bundledPluginPkg;
        ...
        var targetVersion = targetPluginPkg.Version;
        ...
        {
            var dialogBody = "Oculus Utilities has detected that a newer OVRPlugin is available."
            ...
            var dialogResult = EditorUtility.DisplayDialogComplex("Update Oculus Utilities Plugin", dialogBody, "Yes",
        ...
        if (userAcceptsUpdate)
        {
            ...
            if (!targetPluginPkg.IsBundledPluginPackage()) EnablePluginPackage(targetPluginPkg);
