...
public class OVRLint : EditorWindow
{
    ...
    {
        if (state == PlayModeStateChange.EnteredEditMode) ApplyEditModeRequiredFix();
    }
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("Refresh", EditorStyles.toolbarButton, GUILayout.ExpandWidth(false))) RunCheck();
        ...
        for (var x = 0; x < mRecords.Count; x++)
        {
            ...
            if (!record.category.Equals(lastCategory)) // new category
            {
                        ...
                        for (var y = 0; y < record.buttonNames.Length; y++)
                                ...
                                GUILayout.Width(200)))
                            {
                                ...
                                for (var z = x; z < mRecords.Count; z++)
                                {
                                    ...
                                    if (!thisRecord.complete) recordsToProcess.Add(thisRecord);
                ...
                if (moreThanOne || record.targetObject) GUILayout.Label(record.message);
            }
            ...
            if (record.buttonNames != null)
            {
                ...
                for (var y = 0; y < record.buttonNames.Length; y++)
                    ...
                    {
                        if (record.targetObject != null) Undo.RecordObject(record.targetObject, record.category);
    ...
    private static void AddFix(string category, string message, FixMethodDelegate method, Object target,
        bool editModeRequired, params string[] buttons)
    {
    ...
    private static void CheckStaticCommonIssues()
    {
...
#else
        var recommendedPixelLightCount = 3;
#endif

        if (QualitySettings.pixelLightCount > recommendedPixelLightCount)
            AddFix("Optimize Pixel Light Count",
        ...
        if (!PlayerSettings.MTRendering || !PlayerSettings.GetMobileMTRendering(BuildTargetGroup.Android))
            AddFix("Optimize MT Rendering", "For CPU performance, please enable multithreaded rendering.",
                delegate(Object obj, bool last, int selected)
                {
...
#pragma warning restore 618
        ...
        if (tierSettings.renderingPath == RenderingPath.DeferredShading ||
            ...
            AddFix("Optimize Rendering Path", "For CPU performance, please do not use deferred shading.",
                delegate(Object obj, bool last, int selected)
                {
        ...
        if (PlayerSettings.stereoRenderingPath == StereoRenderingPath.MultiPass)
            AddFix("Optimize Stereo Rendering",
        ...
        if (LightmapSettings.lightmaps.Length > 0 && LightmapSettings.lightmapsMode != LightmapsMode.NonDirectional)
            AddFix("Optimize Lightmap Directionality",
        ...
        if (Lightmapping.realtimeGI)
            AddFix("Disable Realtime GI", "Disabling real-time global illumination can improve GPU performance.",
                delegate(Object obj, bool last, int selected) { Lightmapping.realtimeGI = false; }, null, false,
        ...
        for (var i = 0; i < lights.Length; ++i)
        {
            ...
            if (lights[i].shadows != LightShadows.None && !IsLightBaked(lights[i]))
                AddFix("Optimize Shadows", "For CPU performance, consider disabling shadows on realtime lights.",
                    delegate(Object obj, bool last, int selected)
                    {
                        var thisLight = (Light)obj;
        ...
        for (var i = 0; i < clips.Length; ++i)
        {
            if (clips[i].loadType == AudioClipLoadType.DecompressOnLoad)
                AddFix("Audio Loading", "For fast loading, please don't use decompress on load for audio clips",
                    delegate(Object obj, bool last, int selected)
                    {
            ...
            if (clips[i].preloadAudioData)
                AddFix("Audio Preload", "For fast loading, please don't preload data for audio clips.",
                    delegate(Object obj, bool last, int selected) { SetAudioPreload(clips[i], false, last); }, clips[i],
                    false, "Fix");
        }

        if (Physics.defaultContactOffset < 0.01f)
            AddFix("Optimize Contact Offset",
                "For CPU performance, please don't use default contact offset below 0.01.",
                delegate(Object obj, bool last, int selected) { Physics.defaultContactOffset = 0.01f; }, null, false,
                "Fix");

        if (Physics.sleepThreshold < 0.005f)
            AddFix("Optimize Sleep Threshold", "For CPU performance, please don't use sleep threshold below 0.005.",
                delegate(Object obj, bool last, int selected) { Physics.sleepThreshold = 0.005f; }, null, false, "Fix");

        if (Physics.defaultSolverIterations > 8)
            AddFix("Optimize Solver Iterations",
                "For CPU performance, please don't use excessive solver iteration counts.",
                delegate(Object obj, bool last, int selected) { Physics.defaultSolverIterations = 8; }, null, false,
                "Fix");

        var materials = Resources.FindObjectsOfTypeAll<Material>();
        for (var i = 0; i < materials.Length; ++i)
            if (materials[i].shader.name.Contains("Parallax") || materials[i].IsKeywordEnabled("_PARALLAXMAP"))
                AddFix("Optimize Shading", "For GPU performance, please don't use parallax-mapped materials.",
                    delegate(Object obj, bool last, int selected)
                    {
                        var thisMaterial = (Material)obj;
                        if (thisMaterial.IsKeywordEnabled("_PARALLAXMAP")) thisMaterial.DisableKeyword("_PARALLAXMAP");
        ...
        if (overlays.Length > 4)
            AddFix("Optimize VR Layer Count", "For GPU performance, please use 4 or fewer VR layers.",
                delegate(Object obj, bool last, int selected)
                {
                    for (var i = 4; i < OVROverlay.instances.Length; ++i) OVROverlay.instances[i].enabled = false;
                }, null, false, "Fix");
        ...
        if (splashScreen != null)
        {
            if (splashScreen.filterMode != FilterMode.Trilinear)
                AddFix("Optimize VR Splash Filtering",
                    "For visual quality, please use trilinear filtering on your VR splash screen.",
                    delegate(Object obj, bool last, int EditorSelectedRenderState)
                    {
            ...
            if (splashScreen.mipmapCount <= 1)
                AddFix("Generate VR Splash Mipmaps",
    ...
    private static void CheckRuntimeCommonIssues()
    {
        if (!OVRPlugin.occlusionMesh)
            AddFix("Occlusion Mesh",
        ...
        if (OVRManager.instance != null && !OVRManager.instance.useRecommendedMSAALevel)
            AddFix("Optimize MSAA",
                ...
                delegate(Object obj, bool last, int selected)
                {
                    ...
                    foreach (var ovrManager in ovrManagers) ovrManager.useRecommendedMSAALevel = true;
                }, null, true, "Stop Play and Fix");

        if (UnityEngine.XR.XRSettings.eyeTextureResolutionScale > 1.5)
            AddFix("Optimize Render Scale",
    ...
    private static void SetAudioPreload(AudioClip clip, bool preload, bool refreshImmediately)
    {
        if (clip != null)
        {
            ...
            if (importer != null)
                if (preload != importer.preloadAudioData)
                {
                    ...
                    if (refreshImmediately) AssetDatabase.Refresh();
                }
        }
    }

    private static void SetAudioLoadType(AudioClip clip, AudioClipLoadType loadType, bool refreshImmediately)
    {
        if (clip != null)
        {
            var assetPath = AssetDatabase.GetAssetPath(clip);
    ...
    private static T FindComponentInParents<T>(GameObject obj) where T : Component
    {
        T component = null;
        if (obj != null)
        {
