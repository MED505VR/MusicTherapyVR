...
public class OVROverlayCanvasEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (canvas.Opacity == OVROverlayCanvas.DrawMode.TransparentCorrectAlpha)
        {
            ...
            var usingDefaultMaterial = false;
        ...
        canvas.Layer = EditorGUILayout.LayerField(new GUIContent("Layer", "The layer this overlay should be drawn on"),
            canvas.Layer);
        ...
        if (Camera.main != null)
        {
            if ((Camera.main.cullingMask & (1 << canvas.gameObject.layer)) != 0)
            {
                ...
                EditorGUILayout.LabelField("Main Camera does not cull " +
        ...
        else
        {
            var prevColor = GUI.contentColor;
            GUI.contentColor = Color.yellow;
            EditorGUILayout.LabelField("No Main Camera found. Make sure you camera does not draw layer " +
                                       LayerMask.LayerToName(canvas.gameObject.layer));
            GUI.contentColor = prevColor;
        }
        ...
        if (canvas.Layer == canvas.gameObject.layer)
        {
            var prevColor = GUI.contentColor;
            GUI.contentColor = Color.red;
            EditorGUILayout.LabelField("Layer is set to the same layer as this object (" +
                                       LayerMask.LayerToName(canvas.gameObject.layer) + ").");
            GUI.contentColor = prevColor;
        }
