...
namespace Oculus.Platform
{
    ...
    public class OculusPlatformSettingsEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var riftAppIDLabel = new GUIContent("Oculus Rift [?]",
                "This AppID will be used when building to the Windows target.");
            var mobileAppIDLabel = new GUIContent("Oculus Go/Quest or Gear VR [?]",
      ...
      else
      {
        ...
        EditorGUILayout.HelpBox(msg, MessageType.Info);
            ...
            //
            isUnityEditorSettingsExpanded =
                EditorGUILayout.Foldout(isUnityEditorSettingsExpanded, "Unity Editor Settings");
            if (isUnityEditorSettingsExpanded)
                GUIHelper.HInset(6, () =>
                {
                    var HasTestAccessToken =
                    ...
                    var useStandaloneHint =
                        "If this is checked your app will use a debug platform with the User info below.  "
          ...
          GUI.enabled = false;
                    ...
                    PlatformSettings.UseStandalonePlatform =
                        MakeToggle(new GUIContent(useStandaloneLabel, useStandaloneHint),
                            PlatformSettings.UseStandalonePlatform);
 ...
 MakeTextBox(new GUIContent("Bundle Identifier"), PlayerSettings.bundleIdentifier);
...
#endif
                    var canEnableARM64Support = false;
                    ...
                    if (!canEnableARM64Support)
                    {
                        ...
                        if (IsArm64PluginPlatformEnabled()) DisablePluginPlatform(PluginPlatform.Android64);
                    }
                    else
                    {
                        if (!IsArm64PluginPlatformEnabled()) EnablePluginPlatform(PluginPlatform.Android64);
                    }
        ...
        private static string GetPlatformRootPath()
        {
            ...
            var so = CreateInstance(typeof(OculusPluginUpdaterStub));
            var script = MonoScript.FromScriptableObject(so);
            var assetPath = AssetDatabase.GetAssetPath(script);
            var editorDir = Directory.GetParent(assetPath).FullName;
            var platformDir = Directory.GetParent(editorDir).FullName;
        ...
        {
            var path = GetPlatformPluginPath(platform);

            if (!Directory.Exists(path) && !File.Exists(path)) path += PluginDisabledSuffix;
        ...
        {
            var path = GetPlatformPluginPath(PluginPlatform.Android64);
            var pathAlreadyExists = Directory.Exists(path) || File.Exists(path);
        ...
        {
            var path = GetPlatformPluginPath(platform);
            var disabledPath = path + PluginDisabledSuffix;

            var pathAlreadyExists = Directory.Exists(path) || File.Exists(path);
            var disabledPathDoesNotExist = !Directory.Exists(disabledPath) && !File.Exists(disabledPath);

            if (pathAlreadyExists || disabledPathDoesNotExist) return;

            var basePath = GetCurrentProjectPath();
            var relPath = path.Substring(basePath.Length + 1);
            var relDisabledPath = relPath + PluginDisabledSuffix;
        ...
        public static void DisablePluginPlatform(PluginPlatform platform)
        {
            ...
            var pathDoesNotExist = !Directory.Exists(path) && !File.Exists(path);
            var disabledPathAlreadyExists = Directory.Exists(disabledPath) || File.Exists(disabledPath);

            if (pathDoesNotExist || disabledPathAlreadyExists) return;
