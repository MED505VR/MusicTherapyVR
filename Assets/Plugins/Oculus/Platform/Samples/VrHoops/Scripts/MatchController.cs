namespace Oculus.Platform.Samples.VrHoops
{
    ...
    public class MatchController : MonoBehaviour
    {
        ...
        {
            for (var i = 0; i < m_playerAreas.Length; i++) m_playerAreas[i].SetupForPlayer<AIPlayer>("* AI *");
        }
        ...
        private void SetupForPractice()
        {
            ...
            for (var i = 0; i < m_playerAreas.Length; i++)
                if (i == m_localSlot)
        ...
        {
            if (m_currentState == State.NONE) TransitionToState(State.WAITING_TO_START_PRACTICE);
        }
        ...
        {
            foreach (Transform entry in m_mostWinsLeaderboard.transform) Destroy(entry.gameObject);
            foreach (var entry in entries.Values)
            {
                var label = Instantiate(m_leaderboardEntryPrefab);
        ...
        {
            foreach (Transform entry in m_highestScoresLeaderboard.transform) Destroy(entry.gameObject);
            foreach (var entry in entries.Values)
            {
                var label = Instantiate(m_leaderboardEntryPrefab);
        ...
        private void CalculateMatchResults()
        {
            ...
            var wonMatch = localPlayer.Score > remotePlayer.Score;
            PlatformManager.Leaderboards.SubmitMatchScores(wonMatch, localPlayer.Score);

            if (wonMatch) PlatformManager.Achievements.RecordWinForLocalUser();
        }
