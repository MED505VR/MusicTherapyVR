namespace Oculus.Platform.Samples.VrVoiceChat
{
    ...
    public class PlatformManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (m_roomManager.ShouldPollInviteList) m_roomManager.UpdateActiveInvitesList();

            // occasionally send my transform to my interlocutor
            if (m_p2pManager.ShouldSendHeadUpdate) m_p2pManager.SendHeadTransform(m_camera.transform);
        ...
        {
            if (CurrentState == State.WAITING_TO_CALL_OR_ANSWER) m_roomManager.CreateRoomAndLaunchInviteMenu();
        }
        ...
        {
            if (s_instance) s_instance.m_roomManager.JoinExistingRoom(roomID);
        }
        ...
        public static void SetBackgroundColorForState()
        {
            switch (s_instance.m_currentState)
            {
                ...
                case State.CONNECTED_IN_A_ROOM:
                    var red = s_instance.m_p2pManager.Connected ? 1.0f : 0;
                    var green = s_instance.m_voipManager.Connected ? 1.0f : 0;
        ...
        public static void SetActiveInvites(List<RoomManager.Invite> invites)
        {
            if (s_instance && s_instance.m_invitesList && s_instance.m_invitePrefab)
            {
                ...
                foreach (Transform child in s_instance.m_invitesList.transform) Destroy(child.gameObject);

                foreach (var invite in invites)
                {
                    var button = Instantiate(s_instance.m_invitePrefab) as GameObject;
