    ...
    public class DataEntry : MonoBehaviour
    {
        ...
        private void SubmitCommand(string command)
        {
            ...
            if (commandParams.Length > 0)
                ...
                {
                    case "p":
                        if (commandParams.Length > 2) createAndJoinPrivateRoom(commandParams[1], commandParams[2]);
                        break;
                    case "c":
                        getCurrentRoom();
                        break;
                    case "g":
                        if (commandParams.Length > 1) getRoom(commandParams[1]);
                        break;
                    case "j":
                        if (commandParams.Length > 1) joinRoom(commandParams[1]);
                        break;
                    case "l":
                        if (commandParams.Length > 1) leaveRoom(commandParams[1]);
                        break;
                    case "k":
                        if (commandParams.Length > 2) kickUser(commandParams[1], commandParams[2]);
                        break;
                    case "m":
                        getLoggedInUser();
                        break;
                    case "u":
                        if (commandParams.Length > 1) getUser(commandParams[1]);
                        break;
                    case "d":
                        getLoggedInFriends();
                        break;
                    case "i":
                        ...
                        break;
                    case "o":
                        if (commandParams.Length > 2) inviteUser(commandParams[1], commandParams[2]);
                        break;
                    case "s":
                        if (commandParams.Length > 2) setRoomDescription(commandParams[1], commandParams[2]);
                        break;
                    case "w":
                    ...
                    case "e":
                        ...
                        break;
                    case "a":
                        if (commandParams.Length > 1) getAchievementDefinition(commandParams[1]);
                        break;
                    case "b":
                        if (commandParams.Length > 1) getAchievementProgress(commandParams[1]);
                        break;
                    case "3":
                        if (commandParams.Length > 1) unlockAchievement(commandParams[1]);
                        break;
                    case "4":
                        if (commandParams.Length > 2) addCountAchievement(commandParams[1], commandParams[2]);
                        break;
                    case "5":
                        if (commandParams.Length > 2) addFieldsAchievement(commandParams[1], commandParams[2]);
                        break;
                    case "1":
                        if (commandParams.Length > 2) writeLeaderboardEntry(commandParams[1], commandParams[2]);
                        break;
                    case "2":
                        if (commandParams.Length > 1) getLeaderboardEntries(commandParams[1]);
                        break;
                    default:
        ...
        {
            Leaderboards.GetEntries(leaderboardName, 10, LeaderboardFilterType.None, LeaderboardStartAt.Top)
                .OnComplete(leaderboardGetCallback);
        }

        private void writeLeaderboardEntry(string leaderboardName, string value)
        {
            var extraData = new byte[] { 0x54, 0x65, 0x73, 0x74 };

            Leaderboards.WriteEntry(leaderboardName, Convert.ToInt32(value), extraData, false)
                .OnComplete(leaderboardWriteCallback);
        }
        ...
        private void createAndJoinPrivateRoom(string joinPolicy, string maxUsers)
        {
            ...
            Rooms.CreateAndJoinPrivate((RoomJoinPolicy)Convert.ToUInt32(joinPolicy), Convert.ToUInt32(maxUsers))
        ...
        private void kickUser(string roomID, string userID)
        {
            ...
            Rooms.KickUser(Convert.ToUInt64(roomID), Convert.ToUInt64(userID), 10 /*kick duration */)
                .OnComplete(getCurrentRoomCallback);
        }
        ...
        {
            printOutputLine("Room ID: " + room.ID + ", AppID: " + room.ApplicationID + ", Description: " +
                            room.Description);
            var numUsers = room.UsersOptional != null ? room.UsersOptional.Count : 0;
        ...
        private void leaderboardGetCallback(Message<LeaderboardEntryList> msg)
        {
            if (!msg.IsError)
            {
                ...
                foreach (var entry in entries)
                    printOutputLine(entry.Rank + ". " + entry.User.OculusID + " " + entry.Score + " " +
                                    entry.Timestamp);
            }
            else
            {
        ...
        private void achievementProgressCallback(Message<AchievementProgressList> msg)
        {
            if (!msg.IsError)
            {
                ...
                var progressList = msg.GetAchievementProgressList();
        ...
        private void achievementDefinitionCallback(Message<AchievementDefinitionList> msg)
        {
            if (!msg.IsError)
            {
                ...
                var definitionList = msg.GetAchievementDefinitions();
