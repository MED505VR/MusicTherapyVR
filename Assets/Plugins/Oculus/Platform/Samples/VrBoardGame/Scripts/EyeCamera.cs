namespace Oculus.Platform.Samples.VrBoardGame
{
    ...
    public class EyeCamera : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            var hitSomething = Physics.Raycast(transform.position, transform.forward, out hit, 50f);
            if (hitSomething)
            {
            ...
            {
                if (m_eventSystem != null) m_eventSystem.SetSelectedGameObject(null);
                m_currentButton = button;
                if (m_currentButton != null) m_currentButton.Select();
            }

            if (m_currentPiece != piece)
            {
                if (m_currentPiece != null) m_gameController.StoppedLookingAtPiece();
                m_currentPiece = piece;
                if (m_currentPiece != null) m_gameController.StartedLookingAtPiece(m_currentPiece);
            }

            if (m_boardPosition != pos)
            {
                m_boardPosition = pos;
                if (m_boardPosition != null) m_gameController.StartedLookingAtPosition(m_boardPosition);
            }

            // clear the potential move if they gaze off the board
            if (hit.collider == m_gazeTracker) m_gameController.ClearProposedMove();
