namespace Oculus.Platform.Samples.VrBoardGame
{
    ...
    public class MatchmakingManager : MonoBehaviour
    {
        ...
        private void OnApplicationQuit()
        {
            ...
            Matchmaking.Cancel();
            if (m_matchRoom != 0) Rooms.Leave(m_matchRoom);
        }
        ...
        private void MatchmakingEnqueueCallback(Message untyped_msg)
        {
            ...
            var msg = (Message<MatchmakingEnqueueResult>)untyped_msg;
            var info = msg.Data;
        ...
        private void MatchmakingJoinRoomCallback(Message<Room> msg)
        {
            ...
            var numUsers = msg.Data.UsersOptional != null ? msg.Data.UsersOptional.Count : 0;
            ...
            if (msg.Data.UsersOptional != null && msg.Data.UsersOptional.Count == 2) ProcessRoomData(msg.Data);
        }
        ...
        private void MatchmakingRoomUpdateCallback(Message<Room> msg)
        {
            ...
            var ownerOculusID = msg.Data.OwnerOptional != null ? msg.Data.OwnerOptional.OculusID : "";
            var numUsers = msg.Data.UsersOptional != null ? msg.Data.UsersOptional.Count : 0;
        ...
        private void ProcessRoomData(Room room)
        {
            ...
            if (m_state == MatchRoomState.Configuring)
            {
                ...
                var i_go_first = DoesLocalUserGoFirst();
            ...
            // owner of the room.
            if (m_state == MatchRoomState.MyTurn && room.OwnerOptional != null &&
                room.OwnerOptional.ID == PlatformManager.MyID) m_gameController.MarkRemoteTurnComplete();
        ...
        private void ProcessRemoteMove(string moveString)
        {
            ...
            var piece = (GamePiece.Piece)Enum.Parse(typeof(GamePiece.Piece), tokens[0]);
        ...
        {
            var moveString = string.Format("{0}:{1}:{2}", piece.ToString(), boardX, boardY);
        ...
        private bool DoesLocalUserGoFirst()
        {
            ...
            if (m_matchRoom % 2 == 0) return PlatformManager.MyID < m_remotePlayer.ID;
