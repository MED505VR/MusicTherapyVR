namespace Oculus.Platform.Samples.VrBoardGame
{
    ...
    public class GameController : MonoBehaviour
    {
        ...
        private void PerFrameStateUpdate()
        {
            switch (m_state)
            {
                case GameState.PracticingAiTurn:
                    ...
                    if (Random.Range(1, 100) < 3) MakeAIMove(1);
                    break;
        ...
        private void MakeAIMove(int player)
        {
            ...
            var rx = Random.Range(0, GameBoard.LENGTH_X - 1);
            var ry = Random.Range(0, GameBoard.LENGTH_Y - 1);

            // from (rx,ry) search of an available move
            for (var i = 0; i < GameBoard.LENGTH_X && !moved; i++)
            for (var j = 0; j < GameBoard.LENGTH_Y && !moved; j++)
            {
                var x = (rx + i) % GameBoard.LENGTH_X;
                var y = (ry + j) % GameBoard.LENGTH_Y;

                // first try to place a piece on the current position
                if (m_board.CanPlayerMoveToPostion(x, y))
                {
                    var p = Random.Range(0, 2) == 0 ? m_pieceA : m_pieceB;
        ...
        {
            var prefab = m_pieceA.PrefabFor(piece);
        ...
        {
            if (m_state == GameState.OnlineMatchRemoteTurn) TransitionToNextState();
        }
        ...
        public void StartedLookingAtPosition(BoardPosition position)
        {
            ...
            if (newPiece != null)
            {
                if (m_proposedPiece != null) Destroy(m_proposedPiece.gameObject);
