...
*/
    public partial class AudioManager : MonoBehaviour
    {
        ...
        private float
            audioMaxFallOffDistanceSqr = 25.0f * 25.0f; // past this distance, sounds are ignored for the local player 
        ...
        private static readonly AnimationCurve defaultReverbZoneMix =
            new AnimationCurve(new Keyframe[2] { new Keyframe(0f, 1.0f), new Keyframe(1f, 1f) });
        ...
        */
        private void InitializeSoundSystem()
        {
            ...
            var audioListenerObject = FindObjectOfType<AudioListener>() as AudioListener;
            ...
            if (soundEmitterParent != null) // delete any sound emitters hanging around
                Destroy(soundEmitterParent);

            // create them all															
            soundEmitterParent = new GameObject("__SoundEmitters__");
            for (var i = 0; i < CalculateMaxEmittersSize(); i++)
            {
        ...
        */
        private void UpdateFreeEmitters()
        {
            if (verboseLogging)
            {
                if (Input.GetKeyDown(KeyCode.A)) forceShowEmitterCount = !forceShowEmitterCount;
                if (forceShowEmitterCount) showPlayingEmitterCount = true;
            }
            ...
            for (var i = 0; i < playingEmitters.size;)
            {
                ...
                {
                    Debug.LogError(
                        "[AudioManager] ERROR: playingEmitters list had a null emitter! Something nuked a sound emitter!!!");
                ...
                if (!playingEmitters[i].IsPlaying())
                {
                    ...
                    if (verboseLogging)
                        if (nextFreeEmitters.Contains(playingEmitters[i]))
                            Debug.LogError(
                                "[AudioManager] ERROR: playing sound emitter already in the free emitters list!");
            ...
            if (verboseLogging && showPlayingEmitterCount)
            {
                Debug.LogWarning(string.Format(
                    "[AudioManager] Playing sounds: Total {0} | VeryLow {1} | Low {2} | Default {3} | High {4} | VeryHigh {5} | Free {6}",
                    Fmt(total), Fmt(veryLow), Fmt(low), Fmt(def), Fmt(high), Fmt(veryHigh),
                    FmtFree(nextFreeEmitters.Count)));
        ...
        */
        ...
        {
            var t = count / (float)theAudioManager.maxSoundEmitters;
            if (t < 0.5f)
        ...
        */
        private void ResetFreeEmitters()
        {
            ...
            for (var i = (int)EmitterChannel.Any; i < soundEmitters.Length; i++) nextFreeEmitters.Add(soundEmitters[i]);
        }
        ...
        */
        ...
        {
            if (!stopReserved && idx == (int)EmitterChannel.Reserved) return false;
            if (!fadeOut)
        ...
        */
        ...
        {
            for (var i = 0; i < theAudioManager.soundEmitters.Length; i++) StopSound(i, fadeOut, stopReserved);
        }
        ...
        */
        public void MuteAllSounds(bool mute, bool muteReserved = false)
        {
            for (var i = 0; i < soundEmitters.Length; i++)
            {
                if (!muteReserved && i == (int)EmitterChannel.Reserved) continue;
        ...
        */
        public void UnMuteAllSounds(bool unmute, bool unmuteReserved = false)
        {
            for (var i = 0; i < soundEmitters.Length; i++)
            {
                if (!unmuteReserved && i == (int)EmitterChannel.Reserved) continue;
                if (soundEmitters[i].audioSource.isPlaying) soundEmitters[i].audioSource.mute = false;
            }
        }
        ...
        */
        ...
        -----------------------
        */
        public static int PlaySound(AudioClip clip, float volume, EmitterChannel src = EmitterChannel.Any,
            float delay = 0.0f, float pitchVariance = 1.0f, bool loop = false)
        {
            if (!SoundEnabled) return -1;
            return PlaySoundAt(staticListenerPosition != null ? staticListenerPosition.position : Vector3.zero, clip,
                volume, src, delay, pitchVariance, loop);
        }
        ...
        */
        private static int FindFreeEmitter(EmitterChannel src, SoundPriority priority)
        {
            ...
            var next = theAudioManager.soundEmitters[0];
                    ...
                    else
                    {
                        ...
                        if (next == null)
                        {
                            ...
                            else
                            {
                                ...
                                next = theAudioManager.playingEmitters.Find(item =>
                                    item != null && item.priority <= SoundPriority.Default);
                                ;
                            }
                        }
                        ...
                        {
                            if (theAudioManager.verboseLogging)
                                Debug.LogWarning("[AudioManager] cannabalizing " + next.originalIdx + " Time: " +
                                                 Time.time);
            ...
            if (next == null)
            {
                Debug.LogError("[AudioManager] ERROR - absolutely couldn't find a free emitter! Priority = " +
                               priority + " TOO MANY PlaySound* calls!");
        ...
        */
        ...
        {
            if (!SoundEnabled) return -1;
            return PlaySoundAt(staticListenerPosition != null ? staticListenerPosition.position : Vector3.zero, soundFX,
                src, delay);
        }
        ...
        -----------------------
        */
        public static int PlaySoundAt(Vector3 position, SoundFX soundFX, EmitterChannel src = EmitterChannel.Any,
            float delay = 0.0f, float volumeOverride = 1.0f, float pitchMultiplier = 1.0f)
        {
            if (!SoundEnabled) return -1;
            ...
            if (staticListenerPosition != null)
            {
                ...
                if (distFromListener > theAudioManager.audioMaxFallOffDistanceSqr) return -1;
                if (distFromListener > soundFX.MaxFalloffDistSquared) return -1;
            }
            ...
            if (idx == -1) // no free emitters	- should only happen on very low priority sounds
                return -1;
            var emitter = theAudioManager.soundEmitters[idx];
            ...
            audioSource.volume = Mathf.Clamp01(Mathf.Clamp01(theAudioManager.volumeSoundFX * soundFX.volume) *
                                               volumeOverride * soundFX.GroupVolumeOverride);
            ...
            if (src == EmitterChannel.Any) theAudioManager.playingEmitters.AddUnique(emitter);

            // OSP properties
            if (osp != null)
            {
                osp.EnableSpatialization = soundFX.ospProps.enableSpatialization;
                osp.EnableRfl = theAudioManager.enableSpatializedFastOverride || soundFX.ospProps.useFastOverride
                    ? true
                    : false;
            ...
            if (theAudioManager.verboseLogging)
                Debug.Log("[AudioManager] PlaySoundAt() channel = " + idx + " soundFX = " + soundFX.name +
                          " volume = " + emitter.volume + " Delay = " + delay + " time = " + Time.time + "\n");
        ...
        -----------------------
        */
        public static int PlayRandomSoundAt(Vector3 position, AudioClip[] clips, float volume,
            EmitterChannel src = EmitterChannel.Any, float delay = 0.0f, float pitch = 1.0f, bool loop = false)
        {
        ...
        -----------------------
        */
        public static int PlaySoundAt(Vector3 position, AudioClip clip, float volume = 1.0f,
            EmitterChannel src = EmitterChannel.Any, float delay = 0.0f, float pitch = 1.0f, bool loop = false)
        {
            if (!SoundEnabled) return -1;
            ...
            // check the distance from the local player and ignore sounds out of range
            if (staticListenerPosition != null)
                if ((staticListenerPosition.position - position).sqrMagnitude >
                    theAudioManager.audioMaxFallOffDistanceSqr) // no chance of being heard
                    return -1;
            ...
            if (osp != null) osp.EnableSpatialization = false;

            audioSource.transform.position = position;

            if (theAudioManager.verboseLogging)
                Debug.Log("[AudioManager] PlaySoundAt() channel = " + idx + " clip = " + clip.name + " volume = " +
                          emitter.volume + " Delay = " + delay + " time = " + Time.time + "\n");
        ...
        */
        ...
        {
            if (ValidateEmitterIndex(emitterIdx)) theAudioManager.soundEmitters[emitterIdx].SetOnFinished(onFinished);
        }
        ...
        */
        public static void AttachSoundToParent(int idx, Transform parent)
        {
            if (theAudioManager.verboseLogging)
            {
                ...
                if (parent.parent != null) parentName = parent.parent.name + "/" + parentName;
        ...
        */
        ...
        {
            if (theAudioManager.verboseLogging) Debug.Log("[AudioManager] DETACHING INDEX " + idx);
        ...
        */
        ...
        {
            if (theAudioManager != null && idx > -1) theAudioManager.soundEmitters[idx].SetAudioMixer(mixerGroup);
        }
        ...
        */
        public static void SetCurrentSnapshot(MixerSnapshot mixerSnapshot)
        {
            ...
            if (mixerSnapshot == null || mixerSnapshot.snapshot == null)
                ...
                Debug.Log("[AudioManager] Setting audio mixer snapshot: " +
                          (mixerSnapshot != null && mixerSnapshot.snapshot != null
                              ? mixerSnapshot.snapshot.name
                              : "None") + " Time: " + Time.time);
#endif
        ...
        */
        public static void BlendWithCurrentSnapshot(MixerSnapshot blendSnapshot, float weight, float blendTime = 0.0f)
        {
            if (theAudioManager != null)
            {
                ...
                {
                    Debug.LogWarning(
                        "[AudioManager] can't call BlendWithCurrentSnapshot if the audio mixer is not set!");
                    return;
                }

                if (blendTime == 0.0f) blendTime = Time.deltaTime;
                    ...
                    if (blendSnapshot != null && blendSnapshot.snapshot != null)
                    {
                        ...
                        else
                        {
                            var snapshots = new AudioMixerSnapshot[]
                                { theAudioManager.currentSnapshot.snapshot, blendSnapshot.snapshot };
                            var weights = new float[] { 1.0f - weight, weight };
