...
*/
    ...
    public class MixerSnapshotPropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var width = (position.width - 15.0f) / 2.0f;

            // Calculate rects
            var srcRect = new Rect(position.x, position.y, width + 20, position.height);
            position.x += width + 25.0f;
            var destRect = new Rect(position.x, position.y, width - 60, position.height);
            position.x += width - 60.0f;
            ...
            EditorGUI.PropertyField(destRect, property.FindPropertyRelative("transitionTime"),
