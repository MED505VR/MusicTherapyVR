...
*/
    ...
    public class AudioManagerInspector : Editor
    {
        ...
        */
        private void DrawDefaultProperties()
        {
            ...
            if (DrawHeader("Default Properties", true))
            {
                ...
                EditorGUILayout.PropertyField(serializedObject.FindProperty("makePersistent"),
                    new GUIContent("Don't Destroy on Load"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("enableSpatializedAudio"),
                    new GUIContent("Enable Spatialized Audio"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("enableSpatializedFastOverride"),
                    new GUIContent("Force Disable Reflections"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("audioMixer"),
                    new GUIContent("Master Audio Mixer"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("defaultMixerGroup"),
                    new GUIContent("Pooled Emitter Mixer Group"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("reservedMixerGroup"),
                    new GUIContent("Reserved Emitter Mixer Group"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("voiceChatMixerGroup"),
                    new GUIContent("Voice Chat Mixer Group"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("verboseLogging"),
                    new GUIContent("Verbose Logging"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("maxSoundEmitters"),
                    new GUIContent("Max Sound Emitters"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("volumeSoundFX"),
                    new GUIContent("Default Volume"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("soundFxFadeSecs"),
                    new GUIContent("Sound FX Fade Secs"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("audioMinFallOffDistance"),
                    new GUIContent("Minimum Falloff Distance"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("audioMaxFallOffDistance"),
                    new GUIContent("Maximum Falloff Distance"));
        ...
        -----------------------
        */
        private void DrawSoundGroupProperties()
        {
            if (selectedGroup == -1) return;

            var soundGroupsArray = serializedObject.FindProperty("soundGroupings");
            if (selectedGroup >= soundGroupsArray.arraySize) return;
            var soundGroup = soundGroupsArray.GetArrayElementAtIndex(selectedGroup);
            var soundGroupName = soundGroup.FindPropertyRelative("name").stringValue;
            if (DrawHeader(string.Format("{0} Properties", soundGroupName), true))
            {
                EditorGUILayout.BeginVertical(GUI.skin.box);
                EditorGUILayout.PropertyField(soundGroup.FindPropertyRelative("mixerGroup"),
                    new GUIContent("Override Mixer Group",
                        "Leave empty to use the Audio Manager's default mixer group"));
                if (!Application.isPlaying)
                {
                    EditorGUILayout.PropertyField(soundGroup.FindPropertyRelative("maxPlayingSounds"),
                        new GUIContent("Max Playing Sounds Limit",
                            "Max playing sounds for this sound group, 0 = no limit"));
                }
                else
                {
                    ...
                    var playingSounds = soundGroup.FindPropertyRelative("playingSoundCount").intValue;
                    EditorGUILayout.LabelField(string.Format("Playing: {0}", playingSounds), GUILayout.Width(80.0f));
                    EditorGUILayout.EndHorizontal();
                }

                EditorGUILayout.PropertyField(soundGroup.FindPropertyRelative("preloadAudio"),
                    new GUIContent("Preload Audio Clips",
                        "Default = No special preloading, Preload = Audio clips are set to 'Preload', Manual Preload = Audio clips are set to not 'Preload'"));
                EditorGUILayout.PropertyField(soundGroup.FindPropertyRelative("volumeOverride"),
                    new GUIContent("Volume Override",
                        "All sounds played in this group will have volume scaled by this amount"));
                if (soundGroup.FindPropertyRelative("volumeOverride").floatValue == 0.0f)
                    EditorGUILayout.HelpBox("With a volumeOverride of 0.0, these sounds will not play!",
                        MessageType.Warning);
                EditorGUILayout.EndVertical();
                serializedObject.ApplyModifiedProperties();
            }
        ...
        -----------------------
        */
        private void DrawCategories(Event e)
        {
            // do any housework before we start drawing
            if (moveQueued)
            {
                // make a temp copy
                var origSoundList = new List<SoundFX>(audioManager.soundGroupings[origGroup].soundList);
                var temp = origSoundList[origIndex];
                var moveToSoundList = new List<SoundFX>(audioManager.soundGroupings[moveToGroup].soundList);
                ...
                Debug.Log("> Moved '" + temp.name + "' from " + "'" + audioManager.soundGroupings[origGroup].name +
                          "' to '" + audioManager.soundGroupings[moveToGroup].name);
                MarkDirty();
                moveQueued = false;
            }
            ...
            if (addSound)
            {
                var soundList = new List<SoundFX>(audioManager.soundGroupings[selectedGroup].soundList);
            ...
            {
                var soundList = new List<SoundFX>(audioManager.soundGroupings[selectedGroup].soundList);
                var origSoundFX = soundList[dupeSoundIdx];
                // clone this soundFX
                var json = JsonUtility.ToJson(origSoundFX);
                var soundFX = JsonUtility.FromJson<SoundFX>(json);
            ...
            if (e.type == EventType.Repaint) groups.Clear();
            ...
            if (DrawHeader("Sound FX Groups", true))
            {
                EditorGUILayout.BeginVertical(GUI.skin.box);
                soundGroups.Clear();
                for (var i = 0; i < audioManager.soundGroupings.Length; i++)
                    soundGroups.Add(audioManager.soundGroupings[i]);
                for (var i = 0; i < soundGroups.size; i++)
                {
                    EditorGUILayout.BeginHorizontal();
                    {
                        if (i == selectedGroup)
                        ...
                        {
                            // toggle editing
                            if (editGroup >= 0) editGroup = -1;
                            Event.current.Use();
                        }

                        if (i == editGroup)
                            soundGroups[i].name = GUILayout.TextField(soundGroups[i].name,
                                GUILayout.MinWidth(Screen.width - 80f));
                        else
                            GUILayout.Label(soundGroups[i].name,
                                i == selectedGroup ? EditorStyles.whiteLabel : EditorStyles.label,
                                GUILayout.ExpandWidth(true));
                        GUILayout.FlexibleSpace();
                        if (GUILayout.Button(GUIContent.none, "OL Minus", GUILayout.Width(17f))) // minus button
                            if (EditorUtility.DisplayDialog("Delete '" + soundGroups[i].name + "'",
                                "Are you sure you want to delete the selected sound group?", "Continue", "Cancel"))
                            {
                    ...
                    if (e.type == EventType.Repaint) groups.Add(new ItemRect(i, lastRect, null));
                    if (e.type == EventType.MouseDown && lastRect.Contains(e.mousePosition))
                        if (i != selectedGroup || e.clickCount == 2)
                        {
                            ...
                            if (e.clickCount == 2)
                                editGroup = i;
                            else if (editGroup != nextGroup) editGroup = -1;
                            Repaint();
                        }
                }
                ...
                {
                    soundGroups.Sort(delegate(SoundGroup sg1, SoundGroup sg2)
                    {
                ...
                if (soundGroups.size > 0) audioManager.soundGroupings = soundGroups.ToArray();
                ...
                if (e.type == EventType.Repaint && groups.size > 0)
                {
                    ...
                    dropArea.height = groups[groups.size - 1].rect.y - groups[0].rect.y +
                                      groups[groups.size - 1].rect.height;
                }
            }
            ...
            EditorGUILayout.HelpBox(
                "Create and delete sound groups by clicking + and - respectively.  Double click to rename sound groups.  Drag and drop sounds from below to the groups above to move them.",
                MessageType.Info);
        }
        ...
        */
        ...
        -----------------------
        */
        private void DrawSoundEffects(Event e)
        {
            if (selectedGroup < 0 || audioManager.soundGroupings.Length == 0 ||
                selectedGroup >= audioManager.soundGroupings.Length) return;
            ...
            if (DrawHeader("Sound Effects", true))
            {
                ...
                var soundGroupsArray = serializedObject.FindProperty("soundGroupings");
                var soundGroup = soundGroupsArray.GetArrayElementAtIndex(selectedGroup);
                var soundList = soundGroup.FindPropertyRelative("soundList");

                CreateStyles();

                var prevRect = new Rect();
                if (soundList.arraySize > 0) // show all the sounds
                    for (var i = 0; i < soundList.arraySize; i++)
                    {
                        EditorGUI.indentLevel = 1;
                        var soundFX = soundList.GetArrayElementAtIndex(i);
                        var visToggle = soundFX.FindPropertyRelative("visibilityToggle");
                        EditorGUILayout.BeginHorizontal(customDividerStyle);
                        {
                            var soundFXName = soundFX.FindPropertyRelative("name").stringValue;
                        ...
                        if (visToggle.boolValue)
                        {
                            ...
                            if (GUILayout.Button("Delete FX", GUILayout.Width(Screen.width / 3.0f)))
                                if (EditorUtility.DisplayDialog("Delete " + soundFX.displayName, "Are you sure?", "Yes",
                                    "No!"))
                                    deleteSoundIdx = i;
                ...
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                if (GUILayout.Button("Add FX",
                        GUILayout.Width(70f))) //soundList.InsertArrayElementAtIndex( soundList.arraySize );
                    //MarkDirty();
                    addSound = true;
                if (GUILayout.Button("Sort", GUILayout.Width(70f))) sortSounds = true;
                EditorGUILayout.EndHorizontal();
            }
        ...
        -----------------------
        */
        private void CheckStartDrag(Event e)
        {
            if (e.type == EventType.MouseDrag && e.button == 0)
                for (var i = 0; i < items.size; i++)
                    if (items[i].rect.Contains(e.mousePosition))
                    {
                        ...
                        var dragData = new CustomDragData();
        ...
        -----------------------
        */
        private int FindGroupIndex(Event e)
        {
        ...
        */
        ...
        {
            var dragData = DragAndDrop.GetGenericData(dragDropIdentifier) as CustomDragData;
            if (dragData == null) return;

            var groupIndex = FindGroupIndex(e);
        ...
        -----------------------
        */
        private void QueueSoundFXMove(int origGroupIndex, int origSoundIndex, int newGroupIndex)
        {
            moveQueued = true;
        ...
        */
        ...
        public static bool DrawHeader(string text, bool forceOn)
        {
