...
public class OVRLipSyncContextMorphTargetEditor : Editor
{
    ...
    private SerializedProperty visemeToBlendTargets;
    private SerializedProperty visemeTestKeys;
    private SerializedProperty laughterKey;
    private SerializedProperty laughterBlendTarget;
    private SerializedProperty laughterThreshold;
    private SerializedProperty laughterMultiplier;
    private SerializedProperty smoothAmounth;
    ...
    private string[] GetMeshBlendNames()
    {
        ...
        if (morphTarget == null || morphTarget.skinnedMeshRenderer == null) return null;
        var mesh = morphTarget.skinnedMeshRenderer.sharedMesh;
        var blendshapeCount = mesh.blendShapeCount;
        var blendNames = new string[blendshapeCount];
        for (var i = 0; i < mesh.blendShapeCount; ++i) blendNames[i] = mesh.GetBlendShapeName(i);
        return blendNames;
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (EditorGUILayout.PropertyField(visemeToBlendTargets))
        {
            ...
            for (var i = 1; i < visemeNames.Length; ++i)
        ...
        if (morphTarget)
            morphTarget.enableVisemeTestKeys =
                EditorGUILayout.ToggleLeft("Enable Viseme Test Keys", morphTarget.enableVisemeTestKeys);
