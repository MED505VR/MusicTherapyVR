...
namespace OculusSampleFramework
{
    ...
    public class OVROverlaySample : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.A)) TriggerLoad();
        }
        ...
        {
            var numToPrint = 0;
            for (var p = 0; p < numLoopsTrigger; p++) numToPrint++;
        ...
        {
            for (var i = 0; i < spawnedCubes.Count; i++) DestroyImmediate(spawnedCubes[i]);
            spawnedCubes.Clear();
            GC.Collect();
        }
        ...
        public void RadioPressed(string radioLabel, string group, Toggle t)
        {
            if (string.Compare(radioLabel, ovrOverlayID) == 0)
                ...
                ActivateWorldGeo();
            else if (string.Compare(radioLabel, noneID) == 0) ActivateNone();
        }
