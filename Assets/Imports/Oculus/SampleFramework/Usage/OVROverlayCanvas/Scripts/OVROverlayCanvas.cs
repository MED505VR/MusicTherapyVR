...
{
    [SerializeField] [HideInInspector] private Shader _transparentShader = null;
    [SerializeField] [HideInInspector] private Shader _opaqueShader = null;
    ...
    private void Start()
    {
        ...
        var opaqueTrim = Opacity == DrawMode.Opaque
    ...
    protected virtual bool ShouldRender()
    {
        ...
        if (Camera.main != null)
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                if (GeometryUtility.TestPlanesAABB(_FrustumPlanes, _meshRenderer.bounds)) return true;
            }
        ...
        if (ShouldRender())
        {
            if (ScaleViewport)
                if (Camera.main != null)
                {
                    ...
                    var size = PixelsPerUnit * Mathf.Max(_rectTransform.rect.width * transform.lossyScale.x,
