...
namespace OculusSampleFramework
{
    ...
    public class DistanceGrabber : OVRGrabber
    {
        ...
        [SerializeField] public bool m_preventGrabThroughWalls;
        ...
        public override void Update()
        {
            ...
            if (target != m_target)
            {
                if (m_target != null) m_target.Targeted = m_otherHand.m_target == m_target;
                m_target = target;
                m_targetCollider = targetColl;
                if (m_target != null) m_target.Targeted = true;
            }
        }
        ...
        protected bool FindTarget(out DistanceGrabbable dgOut, out Collider collOut)
        {
            ...
            foreach (var cg in m_grabCandidates.Keys)
            {
                ...
                var canGrab = grabbable != null && grabbable.InRange &&
                ...
                for (var j = 0; j < grabbable.grabPoints.Length; ++j)
                {
                    ...
                    if (grabbableMagSq < closestMagSq)
                    {
                        ...
                        if (m_preventGrabThroughWalls)
                        {
                            ...
                            {
                                var distToObject = (grabbableCollider.ClosestPointOnBounds(m_gripTransform.position) -
                                                    m_gripTransform.position).magnitude;
                                if (distToObject > obstructionHitInfo.distance * 1.1) accept = false;
                            }
                        }
            ...
            if (dgOut == null && m_useSpherecast) return FindTargetWithSpherecast(out dgOut, out collOut);
            return dgOut != null;
        }
