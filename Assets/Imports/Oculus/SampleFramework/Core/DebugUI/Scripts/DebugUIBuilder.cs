...
public class DebugUIBuilder : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        for (var i = 0; i < toEnable.Count; ++i) toEnable[i].SetActive(false);
        ...
        for (var i = 0; i < insertedElements.Length; ++i) insertedElements[i] = new List<RectTransform>();

        if (uiHelpersToInstantiate) Instantiate(uiHelpersToInstantiate);
        ...
        if (!toEnable.Contains(lp.gameObject)) toEnable.Add(lp.gameObject);
    ...
    public void Show()
    {
        ...
        len = toEnable.Count;
        for (var i = 0; i < len; ++i) toEnable[i].SetActive(true);

        var numPanels = targetContentPanels.Length;
        for (var i = 0; i < numPanels; ++i) targetContentPanels[i].gameObject.SetActive(insertedElements[i].Count > 0);
    }
    ...
    public void Hide()
    {
        ...
        for (var i = 0; i < len; ++i) toEnable[i].SetActive(false);
    }
    ...
    private void AddRect(RectTransform r, int targetCanvas)
    {
        ...
        if (gameObject.activeInHierarchy) Relayout();
    }
    ...
    {
        var rt = (RectTransform)Instantiate(sliderPrefab);
    ...
    {
        var rt = (RectTransform)Instantiate(dividerPrefab);
        AddRect(rt, targetCanvas);
        return rt;
    }
    ...
    {
        var rt = (RectTransform)Instantiate(togglePrefab);
        AddRect(rt, targetCanvas);
    ...
    {
        var rt = (RectTransform)Instantiate(radioPrefab);
        AddRect(rt, targetCanvas);
        var buttonText = rt.GetComponentInChildren<Text>();
        buttonText.text = label;
