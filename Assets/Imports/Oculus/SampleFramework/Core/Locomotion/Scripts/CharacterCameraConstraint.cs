...
public class CharacterCameraConstraint : MonoBehaviour
{
    ...
    private void CameraUpdate()
    {
        ...
        else if (CheckCameraNearClipping(out clippingOverlap))
        {
            ...
            var fadeParameter = Mathf.InverseLerp(0.0f, FADE_OVERLAP_MAXIMUM, clippingOverlap);
            var fadeAlpha = Mathf.Lerp(0.0f, FADE_AMOUNT_MAXIMUM, fadeParameter);
        ...
        var capsuleOffset = FADE_RAY_LENGTH;

        // Calculate the current height by re-adding the height offset to the camera
        // position.
        var calculatedHeight = CameraRig.centerEyeAnchor.localPosition.y + HeightOffset + capsuleOffset;
        ...
        // changing.
        var calculatedMinimumHeight = MinimumHeight;
        ...
        var calculatedMaximumHeight = MaximumHeight;
        ...
        var cameraRigHeightOffset = HeightOffset - _character.height * 0.5f - capsuleOffset;
    ...
    private bool CheckCameraOverlapped()
    {
        ...
        var yOffset = Mathf.Max(0.0f, _character.height * 0.5f - camera.nearClipPlane - 0.01f);
        ...
        var delta = CameraRig.centerEyeAnchor.position - origin;
    ...
    private bool CheckCameraNearClipping(out float result)
    {
        ...
        var frustumBottomLeft = CameraRig.centerEyeAnchor.position +
                                Vector3.Normalize(CameraRig.centerEyeAnchor.TransformVector(frustumCorners[0])) *
                                FADE_RAY_LENGTH;
        var frustumTopLeft = CameraRig.centerEyeAnchor.position +
                             Vector3.Normalize(CameraRig.centerEyeAnchor.TransformVector(frustumCorners[1])) *
                             FADE_RAY_LENGTH;
        var frustumTopRight = CameraRig.centerEyeAnchor.position +
                              Vector3.Normalize(CameraRig.centerEyeAnchor.TransformVector(frustumCorners[2])) *
                              FADE_RAY_LENGTH;
        var frustumBottomRight = CameraRig.centerEyeAnchor.position +
                                 Vector3.Normalize(CameraRig.centerEyeAnchor.TransformVector(frustumCorners[3])) *
                                 FADE_RAY_LENGTH;
        var frustumCenter = (frustumTopLeft + frustumBottomRight) / 2.0f;

        var hit = false;
        result = 0.0f;
        foreach (var frustumPoint in new Vector3[]
