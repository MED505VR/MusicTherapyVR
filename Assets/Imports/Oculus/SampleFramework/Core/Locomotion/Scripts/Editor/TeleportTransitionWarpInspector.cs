...
public class TeleportTransitionWarpInspector : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Default")) warp.PositionLerp = AnimationCurve.Linear(0, 0, 1, 1);
        if (GUILayout.Button("Ease")) warp.PositionLerp = AnimationCurve.EaseInOut(0, 0, 1, 1);
        if (GUILayout.Button("Step 5")) CreateStep(warp, 5);
        if (GUILayout.Button("Step 10")) CreateStep(warp, 10);
        GUILayout.EndHorizontal();
    }

    private void CreateStep(TeleportTransitionWarp warp, int count)
    {
        var keys = new Keyframe[count + 1];
        for (var i = 0; i < count; i++) keys[i] = new Keyframe((float)i / count, (i + 1.0f) / count);
