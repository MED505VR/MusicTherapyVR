...
namespace OculusSampleFramework
{
    ...
    public class ButtonController : Interactable
    {
        ...
        [SerializeField] private InteractableToolTags[] _allValidToolsTags =
        ...
        protected override void Awake()
        {
            ...
            foreach (var interactableToolTags in _allValidToolsTags) _toolTagsMask |= (int)interactableToolTags;
            ...
            InteractableCollisionDepth newCollisionDepth)
        {
            ...
            if (!isFarFieldTool && _toolToState.Keys.Count > 0 && !_toolToState.ContainsKey(interactableTool)) return;
                ...
                case InteractableState.Default:
                    ...
                    if (validContact && onPositiveSideOfInteractable &&
                        ...
                        upcomingState = newCollisionDepth == InteractableCollisionDepth.Action
                            ? InteractableState.ActionState
                            : InteractableState.ContactState;
                    else if (toolIsInProximity) upcomingState = InteractableState.ProximityState;
        ...
        {
            if (_contactTests == null || collidingTool.IsFarFieldTool) return true;

            foreach (var contactTest in _contactTests)
                switch (contactTest)
                {
                    case ContactTest.BackwardsPress:
                        if (!PassEntryTest(collidingTool, buttonDirection)) return false;

                        break;
                    default:
                        if (!PassPerpTest(collidingTool, buttonDirection)) return false;
        ...
        private bool PassEntryTest(InteractableTool collidingTool, Vector3 buttonDirection)
        {
            ...
            if (dotProduct < ENTRY_DOT_THRESHOLD) return false;
        ...
        private bool PassPerpTest(InteractableTool collidingTool, Vector3 buttonDirection)
        {
            ...
            if (collidingTool.IsRightHandedTool) toolDirection = -toolDirection;

            var dotProduct = Vector3.Dot(toolDirection, buttonDirection);
            if (dotProduct < PERP_DOT_THRESHOLD) return false;
