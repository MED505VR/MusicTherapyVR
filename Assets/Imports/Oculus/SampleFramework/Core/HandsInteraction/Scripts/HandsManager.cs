...
namespace OculusSampleFramework
{
    public class HandsManager : MonoBehaviour
    {
        ...
        private IEnumerator FindSkeletonVisualGameObjects()
        {
            while (!_leftSkeletonVisual || !_rightSkeletonVisual)
            {
                if (!_leftSkeletonVisual)
                {
                    ...
                    if (leftSkeletonVisualTransform) _leftSkeletonVisual = leftSkeletonVisualTransform.gameObject;
                }

                if (!_rightSkeletonVisual)
                {
                    var rightSkeletonVisualTransform = RightHand.transform.Find(SKELETON_VISUALIZER_NAME);
                    if (rightSkeletonVisualTransform) _rightSkeletonVisual = rightSkeletonVisualTransform.gameObject;
                }
        ...
        {
            if (!_leftSkeletonVisual || !_rightSkeletonVisual) return;
