...
public class OVRConfig : ScriptableObject
{
    ...
    [SerializeField] private string gradlePath = "";

    [SerializeField] private string jdkPath = "";
    ...
    public static OVRConfig Instance
    {
        get
        {
            if (instance == null)
            {
                ...
                if (instance == null)
                {
                    instance = CreateInstance<OVRConfig>();
                    var resourcePath = Path.Combine(Application.dataPath, "Resources");
                    if (!Directory.Exists(resourcePath)) AssetDatabase.CreateFolder("Assets", "Resources");

                    var fullPath = Path.Combine(Path.Combine("Assets", "Resources"), "OVRBuildConfig.asset");
    ...
    public string GetAndroidSDKPath(bool throwError = true)
    {
        ...
        if (!Directory.Exists(androidSDKPath))
        {
            ...
            if (!string.IsNullOrEmpty(androidSDKPath)) return androidSDKPath;
...
#if UNITY_2019_1_OR_NEWER
        // Check for use of embedded path or user defined 
        var useEmbedded = EditorPrefs.GetBool("GradleUseEmbedded") ||
...
#else
		libPath =
 Path.Combine(EditorApplication.applicationContentsPath, "PlaybackEngines\\AndroidPlayer\\Tools\\gradle\\lib");
#endif

        libPath = libPath.Replace("/", "\\");
        if (!string.IsNullOrEmpty(libPath) && Directory.Exists(libPath))
        {
            var gradleJar = Directory.GetFiles(libPath, "gradle-launcher-*.jar");
            if (gradleJar.Length == 1) gradlePath = gradleJar[0];
        }
...
#if UNITY_EDITOR_WIN
        // Check for use of embedded path or user defined 
        var useEmbedded = EditorPrefs.GetBool("JdkUseEmbedded") ||
...
#else
			exePath =
 Path.Combine(EditorApplication.applicationContentsPath, "PlaybackEngines\\AndroidPlayer\\Tools\\OpenJDK\\Windows\\bin");
#endif
        }
        else
        {
