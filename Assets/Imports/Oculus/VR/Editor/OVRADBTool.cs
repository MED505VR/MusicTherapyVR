...
public class OVRADBTool
{
    ...
    {
        var tool = new OVRADBTool(androidSdkRoot);
        return tool.isReady;
    }
    ...
    public int StartServer(WaitingProcessToExitCallback waitingProcessToExitCallback)
    {
        ...
        var exitCode = RunCommand(new string[] { "start-server" }, waitingProcessToExitCallback, out outputString,
            out errorString);
        return exitCode;
    }
    ...
    public int KillServer(WaitingProcessToExitCallback waitingProcessToExitCallback)
    {
        ...
        var exitCode = RunCommand(new string[] { "kill-server" }, waitingProcessToExitCallback, out outputString,
            out errorString);
        return exitCode;
    }
    ...
    public int ForwardPort(int port, WaitingProcessToExitCallback waitingProcessToExitCallback)
    {
        ...
        var portString = string.Format("tcp:{0}", port);

        var exitCode = RunCommand(new string[] { "forward", portString, portString }, waitingProcessToExitCallback,
    ...
    public int ReleasePort(int port, WaitingProcessToExitCallback waitingProcessToExitCallback)
    {
        ...
        var exitCode = RunCommand(new string[] { "forward", "--remove", portString }, waitingProcessToExitCallback,
            out outputString, out errorString);
        return exitCode;
    }
    ...
    public List<string> GetDevices()
    {
        ...
        var devices = outputString.Split(new string[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
        ...
        out string outputString, out string errorString)
    {
        ...
        var startInfo = new ProcessStartInfo(adbPath, args);
        ...
        try
        {
            do
            {
                if (waitingProcessToExitCallback != null) waitingProcessToExitCallback();
        ...
        if (!string.IsNullOrEmpty(errorString))
        {
            if (errorString.Contains("Warning"))
                Debug.LogWarning("OVRADBTool " + errorString);
            else
                Debug.LogError("OVRADBTool " + errorString);
        }
