...
public class OVROverlayEditor : Editor
{
        ...
        get
        {
            if (_SrcRectMaterial == null)
            {
                ...
                if (shaders.Length > 0)
                {
                    ...
                    if (shader != null) _SrcRectMaterial = new Material(shader);
                }
            }
        ...
        get
        {
            if (_DestRectMaterial == null)
            {
                ...
                if (shaders.Length > 0)
                {
                    ...
                    if (shader != null) _DestRectMaterial = new Material(shader);
                }
            }
    ...
    public override void OnInspectorGUI()
    {
        ...
        overlay.currentOverlayType = (OVROverlay.OverlayType)EditorGUILayout.EnumPopup(
            new GUIContent("Current Overlay Type",
                "Whether this overlay should layer behind the scene or in front of it"), overlay.currentOverlayType);
        ...
        overlay.noDepthBufferTesting = EditorGUILayout.Toggle(
            new GUIContent("No Depth Buffer Testing",
        ...
        overlay.currentOverlayShape = (OVROverlay.OverlayShape)EditorGUILayout.EnumPopup(
            new GUIContent("Overlay Shape", "The shape of this overlay"), overlay.currentOverlayShape);
        EditorGUILayout.Space();
 ...
 EditorGUILayout.Toggle(new GUIContent("Is External Surface", "On Android, retrieve an Android Surface object to render to (e.g., video playback)"), overlay.isExternalSurface);

		if (lastIsExternalSurface)
		{
			overlay.externalSurfaceWidth =
 EditorGUILayout.IntField("External Surface Width", overlay.externalSurfaceWidth);
			overlay.externalSurfaceHeight =
 EditorGUILayout.IntField("External Surface Height", overlay.externalSurfaceHeight);
			overlay.isProtectedContent =
 EditorGUILayout.Toggle(new GUIContent("Is Protected Content", "The external surface has L1 widevine protection."), overlay.isProtectedContent);
		}
		else
#endif
        {
            if (overlay.textures == null) overlay.textures = new Texture[2];
            if (overlay.textures.Length < 2)
            {
                var tmp = new Texture[2];
                for (var i = 0; i < overlay.textures.Length; i++) tmp[i] = overlay.textures[i];
                overlay.textures = tmp;
            }

            var labelControlRect = EditorGUILayout.GetControlRect();
            EditorGUI.LabelField(
                new Rect(labelControlRect.x, labelControlRect.y, labelControlRect.width / 2, labelControlRect.height),
            ...
            overlay.textures[0] = (Texture)EditorGUI.ObjectField(
                ...
                overlay.textures[0], typeof(Texture), true);
            var right = (Texture)EditorGUI.ObjectField(
                new Rect(textureControlRect.x + textureControlRect.width / 2, textureControlRect.y, 64,
                    textureControlRect.height), overlay.textures[1] != null ? overlay.textures[1] : overlay.textures[0],
                typeof(Texture), true);
            ...
            overlay.isDynamic =
                EditorGUILayout.Toggle(
                    new GUIContent("Dynamic Texture",
                        "This texture will be updated dynamically at runtime (e.g., Video)"), overlay.isDynamic);
#if !UNITY_ANDROID
            ...
            if (lastOverrideTextureRectMatrix)
            {
                ...
                if (sourceRectsVisible)
                {
                    ...
                    if (mat != null)
                    {
                        ...
                        var srcLeft = new Vector4(Mathf.Max(0.0f, overlay.srcRectLeft.x),
                        ...
                        var srcRight = new Vector4(Mathf.Max(0.0f, overlay.srcRectRight.x),
                ...
                if (destRectsVisible)
                {
                    ...
                    if (mat != null)
                    {
                        ...
                        var srcRight = new Vector4(Mathf.Max(0.0f, overlay.srcRectRight.x),
                            ...
                            Mathf.Min(1.0f - overlay.srcRectRight.x, overlay.srcRectRight.width),
                            Mathf.Min(1.0f - overlay.srcRectRight.y, overlay.srcRectRight.height));
                        var destLeft = new Vector4(Mathf.Max(0.0f, overlay.destRectLeft.x),
                        ...
                        var destRight = new Vector4(Mathf.Max(0.0f, overlay.destRectRight.x),
                    ...
                    overlay.destRectLeft = Clamp01(EditorGUI.RectField(
                        new Rect(rectControlRect.x, rectControlRect.y, rectControlRect.width / 2 - 20,
                            rectControlRect.height), overlay.destRectLeft));
                    overlay.destRectRight = Clamp01(EditorGUI.RectField(
                        new Rect(rectControlRect.x + rectControlRect.width / 2, rectControlRect.y,
        ...
        overlay.previewInEditor =
            EditorGUILayout.Toggle(
                new GUIContent("Preview in Editor (Experimental)",
                    "Preview the overlay in the editor using a mesh renderer."), overlay.previewInEditor);
    ...
    {
        if (invertY) selectRect.y = 1 - selectRect.y - selectRect.height;
