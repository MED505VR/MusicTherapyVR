...
public class OVRSkeletonRenderer : MonoBehaviour
{
    ...
    [SerializeField] private IOVRSkeletonRendererDataProvider _dataProvider;
    [SerializeField] private ConfidenceBehavior _confidenceBehavior = ConfidenceBehavior.ToggleRenderer;
    [SerializeField] private SystemGestureBehavior _systemGestureBehavior = SystemGestureBehavior.SwapMaterial;
    [SerializeField] private bool _renderPhysicsCapsules = false;
    [SerializeField] private Material _skeletonMaterial;
    private Material _skeletonDefaultMaterial;
    [SerializeField] private Material _capsuleMaterial;
    private Material _capsuleDefaultMaterial;
    [SerializeField] private Material _systemGestureMaterial = null;
    ...
    private class BoneVisualization
    {
            ...
            SystemGestureBehavior systemGestureBehavior)
        {
            ...
            if (confidenceBehavior == ConfidenceBehavior.ToggleRenderer) Line.enabled = shouldRender;
    ...
    private class CapsuleVisualization
    {
            ...
            OVRBoneCapsule boneCapsule)
        {
            ...
            var collider = CapsuleGO.GetComponent<CapsuleCollider>();
            Destroy(collider);
    ...
    private void Awake()
    {
        if (_dataProvider == null) _dataProvider = GetComponent<IOVRSkeletonRendererDataProvider>();
    ...
    private void Initialize()
    {
        ...
        {
            for (var i = 0; i < _ovrSkeleton.Bones.Count; i++)
            {
            ...
            if (_renderPhysicsCapsules && _ovrSkeleton.Capsules != null)
                for (var i = 0; i < _ovrSkeleton.Capsules.Count; i++)
                {
    ...
    public void Update()
    {
        ...
        if (IsInitialized)
        {
            ...
            if (_dataProvider != null)
            {
                ...
                if (data.IsDataValid) _scale = data.RootScale;
            }

            for (var i = 0; i < _boneVisualizations.Count; i++)
            ...
            for (var i = 0; i < _capsuleVisualizations.Count; i++)
    ...
    {
        if (_skeletonDefaultMaterial != null) DestroyImmediate(_skeletonDefaultMaterial, false);

        if (_capsuleDefaultMaterial != null) DestroyImmediate(_capsuleDefaultMaterial, false);

        if (_systemGestureDefaultMaterial != null) DestroyImmediate(_systemGestureDefaultMaterial, false);
    }
}