...
public class OVRVignette : MonoBehaviour
{
    ...
    [SerializeField] [Tooltip("Controls how the falloff looks.")]
    ...
    [ColorUsage(false)] [Tooltip("The color of the vignette. Alpha value is ignored")]
    ...
    private void BuildMeshes()
    {
        ...
        var innerVerts = new Vector3[triangleCount];
        var innerUVs = new Vector2[triangleCount];
        var outerVerts = new Vector3[triangleCount];
        var outerUVs = new Vector2[triangleCount];
        var tris = new int[triangleCount * 3];
        for (var i = 0; i < triangleCount; i += 2)
        {
            var angle = 2 * i * Mathf.PI / triangleCount;
        ...
        if (_OpaqueMesh != null) DestroyImmediate(_OpaqueMesh);

        if (_TransparentMesh != null) DestroyImmediate(_TransparentMesh);
...
#endif
        if (VignetteShader == null) VignetteShader = Shader.Find("Oculus/OVRVignette");
    ...
    private void Awake()
    {
        ...
        var opaqueObject = new GameObject("Opaque Vignette") { hideFlags = HideFlags.HideAndDontSave };
        ...
        var transparentObject = new GameObject("Transparent Vignette") { hideFlags = HideFlags.HideAndDontSave };
    ...
    private void Update()
    {
        ...
        if (_OpaqueMaterial == null) return;
        ...
        for (var i = 0; i < 2; i++)
        {
            ...
            var borderScale = new Vector2((1 + Mathf.Abs(offsetX)) / VignetteAspectRatio, 1 + Mathf.Abs(offsetY))
                .magnitude * 1.01f;

            var innerScaleX = tanInnerFovX / tanFovX;
            var innerScaleY = tanInnerFovY / tanFovY;
            var middleScaleX = tanMiddleFovX / tanFovX;
            var middleScaleY = tanMiddleFovY / tanFovY;
            var outerScaleX = borderScale * VignetteAspectRatio;
            var outerScaleY = borderScale;
