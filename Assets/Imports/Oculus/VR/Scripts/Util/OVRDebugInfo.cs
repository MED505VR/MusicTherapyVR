...
public class OVRDebugInfo : MonoBehaviour
{
    ...
    private string strFOV = null; // "FOV: 0.0f";
    private string strHeight = null; // "Height: 0.0f";
    private string strDepth = null; // "Depth: 0.0f";
    private string strResolutionEyeTexture = null; // "Resolution : {0} x {1}"
    private string strLatencies = null; // "R: {0:F3} TW: {1:F3} PP: {2:F3} RE: {3:F3} TWE: {4:F3}"
    ...
    private void Update()
    {
        if (initUIComponent && !isInited) InitUIComponents();
    ...
    private void RiftPresentGUI(GameObject guiMainOBj)
    {
        ...
        var rectTransform = riftPresent.GetComponent<RectTransform>();
    ...
    {
        if (riftPresentTimeout >= 0.0f) riftPresentTimeout -= Time.deltaTime;
    }
    ...
    {
        strIPD = string.Format("IPD (mm): {0:F4}", OVRManager.profile.ipd * 1000.0f);
    }
    ...
    {
        var eyeHeight = OVRManager.profile.eyeHeight;
    ...
    {
        var eyeDepth = OVRManager.profile.eyeDepth;
    ...
    {
        var eyeDesc = OVRManager.display.GetEyeRenderDesc(UnityEngine.XR.XRNode.LeftEye);
        strFOV = string.Format("FOV (deg): {0:F3}", eyeDesc.fov.y);
    }
