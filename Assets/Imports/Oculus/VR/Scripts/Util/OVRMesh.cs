...
public class OVRMesh : MonoBehaviour
{
    ...
    [SerializeField] private IOVRMeshDataProvider _dataProvider;
    [SerializeField] private MeshType _meshType = MeshType.None;
    private Mesh _mesh;
    ...
    private void Awake()
    {
        if (_dataProvider == null) _dataProvider = GetComponent<IOVRMeshDataProvider>();

        if (_dataProvider != null) _meshType = _dataProvider.GetMeshType();

        if (ShouldInitialize()) Initialize(_meshType);
    }
    ...
    private void Initialize(MeshType meshType)
    {
        ...
        if (OVRPlugin.GetMesh((OVRPlugin.MeshType)_meshType, out ovrpMesh))
        {
            ...
            for (var i = 0; i < ovrpMesh.NumVertices; ++i)
            ...
            for (var i = 0; i < ovrpMesh.NumIndices; ++i) triangles[i] = ovrpMesh.Indices[ovrpMesh.NumIndices - i - 1];
