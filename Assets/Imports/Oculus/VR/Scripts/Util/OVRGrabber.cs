...
public class OVRGrabber : MonoBehaviour
{
    ...
    {
        var canRelease = m_grabbedObj != null &&
                         m_grabbedObj == grabbable;
        if (canRelease) GrabEnd();
    }
    ...
    protected virtual void Awake()
    {
        ...
        if (!m_moveHandPosition)
        {
            ...
            var rig = transform.GetComponentInParent<OVRCameraRig>();
            if (rig != null)
            {
    ...
    protected virtual void Start()
    {
        ...
        if (m_parentTransform == null) m_parentTransform = gameObject.transform;
    ...
    {
        if (m_operatingWithoutOVRCameraRig) OnUpdatedAnchors();
    }
    ...
    {
        var destPos = m_parentTransform.TransformPoint(m_anchorOffsetPosition);
        var destRot = m_parentTransform.rotation * m_anchorOffsetRotation;
        ...
        if (!m_parentHeldObject) MoveGrabbedObject(destPos, destRot);
    ...
    {
        if (m_grabbedObj != null) GrabEnd();
    }

    private void OnTriggerEnter(Collider otherCollider)
    {
        // Get the grab trigger
        var grabbable = otherCollider.GetComponent<OVRGrabbable>() ??
    ...
    private void OnTriggerExit(Collider otherCollider)
    {
        ...
        var refCount = 0;
        var found = m_grabCandidates.TryGetValue(grabbable, out refCount);
        if (!found) return;
    ...
    protected virtual void GrabBegin()
    {
        ...
        foreach (var grabbable in m_grabCandidates.Keys)
        {
        ...
        {
            if (closestGrabbable.isGrabbed) closestGrabbable.grabbedBy.OffhandGrabbed(closestGrabbable);
    ...
    {
        if (m_grabVolumeEnabled == enabled) return;

        m_grabVolumeEnabled = enabled;
        for (var i = 0; i < m_grabVolumes.Length; ++i)
        {
            var grabVolume = m_grabVolumes[i];
            grabVolume.enabled = m_grabVolumeEnabled;
        }

        if (!m_grabVolumeEnabled) m_grabCandidates.Clear();
    }

    protected virtual void OffhandGrabbed(OVRGrabbable grabbable)
    {
        if (m_grabbedObj == grabbable) GrabbableRelease(Vector3.zero, Vector3.zero);
    }
    ...
    protected void SetPlayerIgnoreCollision(GameObject grabbable, bool ignore)
    {
        if (m_player != null)
        {
            var playerColliders = m_player.GetComponentsInChildren<Collider>();
