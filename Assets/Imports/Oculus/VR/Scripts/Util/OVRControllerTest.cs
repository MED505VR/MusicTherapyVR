...
public class OVRControllerTest : MonoBehaviour
{
    ...
    private void Start()
    {
        if (uiText != null) uiText.supportRichText = false;
    ...
    private static BoolMonitor controllers = new BoolMonitor("Controllers Changed",
    ...
    private void Update()
    {
        ...
        data.Length = 0;
        var battery = OVRInput.GetControllerBatteryPercentRemaining();
        data.AppendFormat("Battery: {0}\n", battery);

        var framerate = OVRPlugin.GetAppFramerate();
        ...
        var rot = OVRInput.GetLocalControllerRotation(activeController);
        data.AppendFormat("Orientation: ({0:F2}, {1:F2}, {2:F2}, {3:F2})\n", rot.x, rot.y, rot.z, rot.w);

        var angVel = OVRInput.GetLocalControllerAngularVelocity(activeController);
        data.AppendFormat("AngVel: ({0:F2}, {1:F2}, {2:F2})\n", angVel.x, angVel.y, angVel.z);

        var angAcc = OVRInput.GetLocalControllerAngularAcceleration(activeController);
        data.AppendFormat("AngAcc: ({0:F2}, {1:F2}, {2:F2})\n", angAcc.x, angAcc.y, angAcc.z);

        var pos = OVRInput.GetLocalControllerPosition(activeController);
        data.AppendFormat("Position: ({0:F2}, {1:F2}, {2:F2})\n", pos.x, pos.y, pos.z);

        var vel = OVRInput.GetLocalControllerVelocity(activeController);
        data.AppendFormat("Vel: ({0:F2}, {1:F2}, {2:F2})\n", vel.x, vel.y, vel.z);

        var acc = OVRInput.GetLocalControllerAcceleration(activeController);
        data.AppendFormat("Acc: ({0:F2}, {1:F2}, {2:F2})\n", acc.x, acc.y, acc.z);

        var indexTrigger = OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger);
        data.AppendFormat("PrimaryIndexTriggerAxis1D: ({0:F2})\n", indexTrigger);

        var handTrigger = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger);
        ...
        if (uiText != null) uiText.text = data.ToString();
    }
}