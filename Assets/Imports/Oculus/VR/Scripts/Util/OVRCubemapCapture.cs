...
public class OVRCubemapCapture : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(triggeredByKey)) TriggerCubemapCapture(transform.position, cubemapSize, pathName);
    }
    ...
    {
        var width = (int)outCubemap.width;
        var height = (int)outCubemap.height;
