...
public class OVRCustomSkeleton : OVRSkeleton
{
    ...
    {
        var start = GetCurrentStartBoneId();
        var end = GetCurrentEndBoneId();
        var skeletonType = GetSkeletonType();
        if (start != BoneId.Invalid && end != BoneId.Invalid)
            for (var bi = (int)start; bi < (int)end; ++bi)
            {
                var fbxBoneName = FbxBoneNameFromBoneId(skeletonType, (BoneId)bi);
                var t = transform.FindChildRecursive(fbxBoneName);
    ...
    {
        {
            if (bi >= BoneId.Hand_ThumbTip && bi <= BoneId.Hand_PinkyTip)
                return _fbxHandSidePrefix[(int)skeletonType] +
                       _fbxHandFingerNames[(int)bi - (int)BoneId.Hand_ThumbTip] + "_finger_tip_marker";
            else
