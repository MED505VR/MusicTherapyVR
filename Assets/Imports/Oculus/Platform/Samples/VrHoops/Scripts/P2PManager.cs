namespace Oculus.Platform.Samples.VrHoops
{
    ...
    public class P2PManager
    {
        ...
        private readonly Dictionary<ulong, RemotePlayerData>
            m_remotePlayers = new Dictionary<ulong, RemotePlayerData>();
        ...
        public void UpdateNetwork()
        {
            ...
            if (Time.time >= m_timeForNextBallUpdate && m_localBalls.Count > 0) SendLocalBallTransforms();
        }
        ...
        {
            foreach (var id in m_remotePlayers.Keys) Net.Close(id);
            ...
            if (m_remotePlayers.ContainsKey(msg.Data.ID))
            {
                ...
                switch (msg.Data.State)
                {
                    case PeerConnectionState.Connected:
                        if (PlatformManager.MyID < msg.Data.ID) SendTimeSyncMessage(msg.Data.ID);
                        break;

                    case PeerConnectionState.Timeout:
                        if (PlatformManager.MyID < msg.Data.ID) Net.Connect(msg.Data.ID);
                        break;
        ...
        {
            if (!m_remoteSyncTimeCache.ContainsKey(remoteID)) m_remoteSyncTimeCache[remoteID] = new List<float>();

            var time = Time.realtimeSinceStartup;
            m_remoteSentTimeCache[remoteID] = time;

            var buf = new byte[TIME_SYNC_MESSAGE_SIZE];
        ...
        private void ReadTimeSyncMessage(ulong remoteID, byte[] msg)
        {
            ...
            var offset = 1;
            var remoteTime = UnpackFloat(msg, ref offset);
            var now = Time.realtimeSinceStartup;
            var latency = (now - m_remoteSentTimeCache[remoteID]) / 2;
            var remoteTimeOffset = now - (remoteTime + latency);
            ...
            else
            {
                ...
                var median = m_remoteSyncTimeCache[remoteID][TIME_SYNC_MESSAGE_COUNT / 2];

                // calucate the mean and standard deviation
                double mean = 0;
                foreach (var time in m_remoteSyncTimeCache[remoteID]) mean += time;
                mean /= TIME_SYNC_MESSAGE_COUNT;

                double std_dev = 0;
                foreach (var time in m_remoteSyncTimeCache[remoteID]) std_dev += (mean - time) * (mean - time);
        ...
        {
            var buf = new byte[START_TIME_MESSAGE_SIZE];
        ...
        {
            var offset = 1;
            var remoteTime = UnpackTime(remoteID, msg, ref offset);
        ...
        {
            var buf = new byte[BACKBOARD_UPDATE_MESSAGE_SIZE];
        ...
        private void ReceiveBackboardUpdate(ulong remoteID, byte[] msg)
        {
            var offset = 1;
            var remoteTime = UnpackTime(remoteID, msg, ref offset);
            var pos = UnpackVector3(msg, ref offset);
            var moveDir = UnpackVector3(msg, ref offset);
            var nextMoveDir = UnpackVector3(msg, ref offset);
        ...
        private void SendLocalBallTransforms()
        {
            ...
            var msgSize = 1 + 4 + m_localBalls.Count * (1 + 4 + 12 + 12);
        ...
        private void ReceiveBallTransforms(ulong remoteID, byte[] msg, ulong msgLength)
        {
            ...
            {
                var isHeld = UnpackBool(msg, ref offset);
                var instanceID = UnpackInt32(msg, ref offset);
                var pos = UnpackVector3(msg, ref offset);
                var vel = UnpackVector3(msg, ref offset);
                ...
                if (ball) ball.ProcessRemoteUpdate(remoteTime, isHeld, pos, vel);
            }
        }
        ...
        {
            var buf = new byte[SCORE_UPDATE_MESSAGE_SIZE];
        ...
        {
            var offset = 1;
            var score = UnpackUint32(msg, ref offset);
        ...
        {
            var value = BitConverter.ToUInt32(buf, offset);
            offset += 4;
            return value;
        }
